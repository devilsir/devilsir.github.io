...
import { MimicBoss } from "../entities/MimicBoss.js";
export const CHAPTER4_ITEMS = {
...
export class Chapter4World {
    ...
    falseVoiceEvents = new Set();
    recoveredEquipment = new Set();
    mimicEvidence = new Set();
    collectedDocuments = new Set();
    collectedRecordings = new Set();
    floorTiles = [];
    doors = [];
    barriers = [];
    falseVoices = [];
    ...
    arenaPointLights = [];
    mirrors = [];
    removedLoadout = new Set();
    floorResetTimer = 0;
    floorCheckTimer = 0;
    floorSafeIndex = -1;
    guardObservationClock = 0;
    guardWindowOpen = false;
    falseVoiceClock = 2;
    currentVoiceIndex = 0;
    choiceLocked = false;
    bloodEncounterClock = -1;
    ...
    mechanicalLock;
    maintenanceDoor;
    maintenanceDoorBlocker;
    equipmentLocker;
    bloodTrapDoor;
    bodyInspectMesh;
    archiveExitConsole;
    arenaMirrorRefreshClock = 0;
    corruptionClock = 0;
    lastSafeCheckpoint = "prisonCell";
    ...
    constructor(scene, materials, interaction, inventory, objective, ui, audio, fire, player, settings, callbacks) {
        ...
        this.boss = new MimicBoss(scene, this.mimic, materials, player, fire, audio, ui, settings, this.checkpoints.mimicArena, {
    ...
    restore(progress, checkpoint) {
        ...
        if (this.chapterComplete)
            this.phase = "complete";
        else if (progress.bossState !== "not-started")
            this.phase = progress.bossState === "stopped" || progress.bossState === "complete" ? "postboss" : "boss";
        else if (this.equipmentChoice === "recovered")
            this.phase = "preboss";
        else if (this.fragileFloorSolved)
            this.phase = "choice";
        else if (this.prisonEscaped)
            this.phase = "floor";
        else
            this.phase = "escape";
        ...
        if (this.phase === "escape")
            ...
            this.objective.set("cross-fragile-floor", "ATRAVESSE O PISO DE TESTE.");
        ...
        this.boss.restorePostDefeatStats(progress.postDefeatAttackCount, progress.postDefeatStopTime, progress.inspectedBody, progress.attemptedImmediateExit);
        if (progress.bossState === "post-defeat")
            this.onBossApparentlyDefeated();
    }
    serialize() {
        return {
    ...
    destinationForCheckpoint(checkpoint) {
        ...
        if (checkpoint.startsWith("mimic-"))
            return this.checkpoints.mimicArena.clone();
        if (checkpoint === "floor-complete")
            return this.checkpoints.choice.clone();
        if (checkpoint === "equipment-recovered")
            return this.checkpoints.identityTesting.clone();
        if (checkpoint === "bad-ending-retry")
            return this.checkpoints.choice.clone();
        if (checkpoint === "chapter5-transition")
            return this.checkpoints.chapter5.clone();
    ...
    handlePrimaryAttack(charged) {
        ...
        if (this.phase === "boss" || this.boss.isPostDefeat())
            return this.boss.handleAttack(charged, this.fire.torchLit);
    ...
    handleContextAction() {
        if (!this.active)
            return false;
        if (this.phase === "boss")
            return this.boss.handleTacticalAction("defend");
        return false;
    }
    ...
    forceIdentityInconsistency(evidence) {
        this.enableForDebug();
    ...
    createObservationAndInterrogation() {
    ...
    createFragileFloorPuzzle() {
        const tileMaterial = this.materials.get("tile", 32);
        const hiddenBase = this.scene.meshes.find((mesh) => mesh.name === "fragile-floor-shell-floor");
        if (hiddenBase) {
    ...
    createPreBossEncounters() {
        this.preBossEncounters.push({ id: "maya-reflection", position: new Vector3(-12, 0, 1328), identity: "maya", evidence: "mirrored-hand", triggered: false }, { id: "daniel-recording", position: new Vector3(12, 0, 1328), identity: "daniel", evidence: "wrong-memory", triggered: false }, { id: "employee-breath", position: new Vector3(0, 0, 1340), identity: "employee", evidence: "shared-breath", triggered: false });
    ...
    createMimicArena() {
        ...
        for (let index = 0; index < 6; index += 1) {
            ...
            const light = new SpotLight(`mimic-arena-spot-${index}`, this.checkpoints.mimicArena.add(new Vector3(Math.sin(angle) * 17, 8.5, Math.cos(angle) * 17)), new Vector3(-Math.sin(angle), -0.75, -Math.cos(angle)), Math.PI / 4, 2, this.scene);
        ...
        stations.forEach((station, index) => {
            ...
            this.interaction.register(console, {
                ...
                onInteract: () => {
                    ...
                    if (light)
                        light.intensity = 9;
                    window.setTimeout(() => { if (light)
                        light.intensity = 5.5; }, 900);
                }
            });
        });
    }
    createDocumentsAndRecordings() {
        this.createDocument("permanent-visitors", "VISITANTES PERMANENTES · NORMA 6", "O visitante que demonstra retenção de identidade após o terceiro teste não deverá retornar ao setor público. O termo permanente descreve permanência de observação, não hospedagem.", new Vector3(-5.3, 1.1, 1098));
        this.createDocument("floor-test-record", "REGISTRO DE PISO 17-B", "Os símbolos foram fotografados através do vidro de observação. A sequência registrada no prontuário está invertida. Não corrigir a cópia infantil: ela é a única representação fiel ao ponto de vista do sujeito.", new Vector3(10.6, 1.2, 1188));
        this.createDocument("identity-protocol", "PROTOCOLO DE IDENTIDADE", "Perguntas pessoais não são suficientes. Confirmar dominância manual, direção da voz, sincronia respiratória, reflexo ocular e continuidade de vestuário. Uma resposta correta pode ser reproduzida; um corpo coerente é mais difícil.", new Vector3(-18, 1.15, 1318));
        this.createDocument("mimic-postmortem", "OBSERVAÇÃO PÓS-COLAPSO", "A interrupção voluntária do estímulo agressivo produz respostas diferentes da exaustão física. Registrar número de repetições, tempo de cessação e tentativa de inspeção. Não informar ao sujeito que o teste continua após a queda.", new Vector3(18, 1.15, 1402));
    ...
    createFalseVoiceCues() {
        this.falseVoices.push({ id: "maya-behind-wall", position: new Vector3(-5, 1.5, 1105), speaker: "Maya", text: "É por aqui. Eu lembro deste corredor.", variant: "call", radius: 9, played: false }, { id: "daniel-impossible", position: new Vector3(18, 1.5, 1138), speaker: "Daniel", text: "Não pega o porrete. Eu ainda estou usando.", variant: "intercom", radius: 10, played: false }, { id: "child-cry", position: new Vector3(0, 1.1, 1175), speaker: "Criança", text: "O círculo cai. A cruz é segura. O círculo cai.", variant: "cry", radius: 10, played: false }, { id: "protagonist-voice", position: new Vector3(8, 1.5, 1238), speaker: "Sua voz", text: "Segue o sangue. Você já decidiu.", variant: "call", radius: 11, played: false }, { id: "chorus-route", position: new Vector3(0, 1.5, 1308), speaker: "Vozes", text: "Maya está viva. Daniel está vivo. Você está vivo. Repete.", variant: "chorus", radius: 13, played: false });
    }
    updateObservationWindow() {
        ...
        if (open !== this.guardWindowOpen) {
            ...
            if (light)
                light.intensity = open ? 0.08 : 4.2;
            if (this.escapeMechanism[1] && !this.escapeMechanism[2] && open)
                this.ui.showSoundCaption("a lâmpada de observação apaga por um instante");
        }
    }
    updateFalseVoices() {
        for (const cue of this.falseVoices) {
            if (cue.played || this.falseVoiceEvents.has(cue.id))
                continue;
            if (Vector3.Distance(this.player.collider.position, cue.position) > cue.radius)
                continue;
            cue.played = true;
            ...
            if (cue.id === "child-cry")
                this.ui.showSoundCaption("a voz termina com uma palavra reproduzida ao contrário");
            if (cue.id === "protagonist-voice")
                this.ui.showSoundCaption("sua própria voz vem de duas paredes diferentes");
        }
        if (this.falseVoiceClock > 0 || this.phase === "boss" || this.phase === "complete")
            return;
        ...
        if (Math.random() < 0.45)
            this.ui.showSoundCaption("uma voz conhecida chama de um ponto impossível");
    }
    updateFloorTiles(deltaSeconds) {
        ...
        if (!tile)
            return;
        if (tile.index === this.floorSafeIndex)
            return;
        ...
        if (tile.crack >= allowed)
            this.beginTileCollapse(tile, false);
    }
    beginTileCollapse(tile, immediate) {
        if (tile.falling || tile.collapsed)
            return;
        tile.falling = true;
    ...
    failFragileFloor() {
        if (!this.active || this.fragileFloorSolved)
            return;
    ...
    updateBloodEncounter(deltaSeconds) {
        if (this.bloodEncounterClock < 0)
            return;
        ...
        if (this.bloodEncounterClock > 2.3 && !this.badEndingActive)
            this.beginBadEnding();
    }
    updatePreBossEncounters() {
        if (this.phase !== "preboss")
            return;
        for (const encounter of this.preBossEncounters) {
            if (encounter.triggered || Vector3.Distance(this.player.collider.position, encounter.position) > 5.2)
                continue;
    ...
    updateArenaMirrors() {
        if (this.arenaMirrorRefreshClock > 0)
            return;
    ...
    updateAreaProgression() {
        ...
        if (this.phase === "preboss" && position.z > 1350 && this.equipmentRecovered)
            this.beginBoss();
        if (this.phase === "postboss" && position.z > 1442 && !this.chapterExitTriggered)
            this.finishChapter();
    }
    recoverEquipment() {
    ...
    beginBadEnding() {
        ...
        window.setTimeout(() => {
            this.ui.showEnding("FINAL: MORTO PELA CRIATURA.", "O rastro não levava a Maya. Sem a tocha, o porrete e os cartões, o caminho seguro termina em uma sala sem saída.", () => this.retryAfterBadEnding(), () => this.callbacks.onReturnMenu());
        }, 950);
    }
    retryAfterBadEnding() {
    ...
    onBossStopped(stats) {
        ...
        if (stats.attacks > 0)
            this.ui.toast("As máquinas registram os últimos impactos sem emitir julgamento.", 3200);
    }
    openArchiveRoute() {
        this.phase = "postboss";
    ...
    stripEquipment() {
        ["metalClub", "replacementTorch", "portableRecorder", "bodyCard", "generatorAccessCard", "metalCan", "torch"].forEach((id) => {
            if (this.inventory.has(id))
                this.removedLoadout.add(id);
            this.inventory.remove(id);
        });
        this.fire.torchLit = false;
    }
    restoreEquipmentFromProgress() {
        const catalog = {
        ...
        this.recoveredEquipment.forEach((id) => { if (catalog[id])
            this.inventory.add(catalog[id]); });
    ...
    damagePlayer(amount, caption) {
        ...
        if (this.phase === "boss")
            this.boss.start(this.boss.state === "post-defeat" ? "post-defeat" : this.boss.state, Math.max(35, this.boss.health), this.boss.identityRoundsCompleted);
    }
    ...
    createDoor(name, position, slide) {
        ...
        const rig = { mesh, blocker, base: position.clone(), slide: new Vector3(4.7, 0, 0), open: false };
        this.doors.push(rig);
        return rig;
    }
    createDoorFrame(name, position, width, height, rotationY, openingSpan = width, wallHeight = height + 0.6) {
        ...
        if (upperHeight > 0.08)
            make("wall-infill-upper", { width: Math.max(openingSpan, width + jamb * 2), height: upperHeight, depth: 0.62 }, new Vector3(0, height / 2 + upperHeight / 2, 0));
    ...
    createDamagedReflectivePanel(position, normal, width, height, variant) {
    ...
    createArenaMirror(position, normal, width, height, variant) {
    ...
    createCrackTexture(name, variant) {
        ...
        for (let dot = 0; dot < 80; dot += 1)
            context.fillRect((dot * 73 + variant * 17) % 512, (dot * 41 + variant * 29) % 512, 2 + dot % 5, 2 + dot % 3);
        texture.update();
        return texture;
    }
    createProjectorSlideClue() {
    ...
    drawFloorSymbol(tile, symbol, index) {
    ...
    createProceduralSign(name, text, position, rotationY, width, height, warning) {
