...
import { PlushEnemy } from "../entities/PlushEnemy";
import type { BodyBoss } from "../entities/BodyBoss";


interface LootContainerRig {
  hinge: TransformNode;
  door: Mesh;
  contents: Mesh[];
...
export interface WorldCallbacks {
  onEnteredWonderWorld: () => void;
  onPowerRestored: () => void;
  onPuzzleSolved: (id: string) => void;
  onBossRequested: () => void;
  onBodyCardObtained: () => void;
  onChapterTransition: () => void;
  onPlayerDamaged: () => void;
  onCheckpointActivated: (id: string, position: Vector3) => void;
}
...
export class WorldBuilder {
  ...
  public enteredWonderWorld = false;
  public fenceOpened = false;
  public sideDoorOpened = false;
  public auditoriumOpened = false;
  public bodyDefeated = false;
  public plushEnemies: PlushEnemy[] = [];
  public solvedPuzzles = new Set<string>();
  public sideDoor!: Mesh;
  public auditoriumDoor!: Mesh;
  public elevatorDoor!: Mesh;
  public bodyCardMesh: Mesh | null = null;
  private sideDoorBlocker!: Mesh;
  private auditoriumDoorBlocker!: Mesh;
  private elevatorDoorBlocker!: Mesh;
  ...
  private boss: BodyBoss | null = null;
  private readonly coverSeats: Mesh[] = [];
  private readonly coverSeatDamage = new Map<number, number>();
  private readonly stageCurtains: Mesh[] = [];
  private keyTaken = false;
  private crateMoved = false;
  private lockerOpened = false;
  private readonly openedCarTrunks = new Set<number>();
  private readonly fuelCarIndex = 2;
  private torchTaken = false;
  private fuelTaken = false;
  private cableTaken = false;
  private fuseTaken = false;
  private crankTaken = false;
  private panelKeyTaken = false;
  private bandageTaken = false;
  private handsAngles = [0, 0, 0, 0];
  private eyesAngles = [0, 0, 0];
  private heartValves = [0, 0, 0];
  private heartPulses: number[] = [];
  private feetProgress = 0;
  private readonly feetSequence = [0, 2, 1, 3];
  private spotlightActivations = 0;
  private auditoriumTriggerUsed = false;
  private readonly carHeadlightSets: Array<{ bulbs: Mesh[]; lights: SpotLight[]; material: PBRMaterial }> = [];
  private carHeadlightFlashToken = 0;
  private meleeCooldownUntil = 0;
  private readonly openedLootContainers = new Set<string>();
  private readonly lootedContainers = new Set<string>();
  private readonly activatedCheckpoints = new Set<string>();
  private readonly lootContainerRigs = new Map<string, LootContainerRig>();
  private readonly survivalCheckpointPositions = new Map<string, Vector3>();
  ...
  public attachBoss(boss: BodyBoss): void {
    this.boss = boss;
  }

  public update(): void {
    if (!this.enteredWonderWorld) return;
  ...
  public handlePrimaryAttack(charged: boolean): boolean {
    if (!this.inventory.has("crowbar") || !this.player.enabled) return false;
    const now = performance.now();
    if (now < this.meleeCooldownUntil) return true;
    ...
    let selected: PlushEnemy | null = null;
    ...
    for (const enemy of this.plushEnemies) {
      ...
      if (distance <= 0.01 || distance > bestDistance) continue;
      const facing = Vector3.Dot(forward, offset.scale(1 / distance));
      if (facing < (charged ? 0.42 : 0.58)) continue;
      selected = enemy;
  ...
  public getGuideRoute(objectiveId: string): Vector3[] | null {
    const targetByObjective: Record<string, Vector3> = {
    ...
    if (!this.enteredWonderWorld) {
      if (target.z >= 37) return [new Vector3(20, 0.12, 31.5), target.clone()];
      return [target.clone()];
    }
  ...
  private buildInteriorGuideRoute(player: Vector3, target: Vector3): Vector3[] {
    const route: Vector3[] = [];
    ...
    const portalFor = (point: Vector3): Vector3 | null => {
      for (const portal of portals) {
        const [minX, maxX, minZ, maxZ] = portal.bounds;
        if (point.x >= minX! && point.x <= maxX! && point.z >= minZ! && point.z <= maxZ!) return portal.point.clone();
      }
      return null;
    };
    const push = (point: Vector3): void => {
      const previous = route[route.length - 1];
      if (!previous || Vector3.Distance(previous, point) > 0.55) route.push(point.clone());
    };
    ...
    if (target.z >= 60 && target.z <= 186) push(new Vector3(0, y, target.z));
    push(new Vector3(target.x, y, target.z));
    return route;
  }
  ...
  public getOpenedLootContainers(): string[] {
    return [...this.openedLootContainers];
  }

  public getLootedContainers(): string[] {
    return [...this.lootedContainers];
  }

  public getActivatedCheckpoints(): string[] {
  ...
  public restoreProgress(
    ...
    const interiorCheckpoint =
      checkpoint === "chapter1-entry"
    ...
    if (powerRestored) this.enableFacilityLights(true);
    if (this.solvedPuzzles.size >= 4) this.openAuditorium();
  }

  public resetNearestPuzzle(): void {
    ...
    if (nearest === "hands") this.handsAngles = [0, 0, 0, 0];
    if (nearest === "eyes") this.eyesAngles = [0, 0, 0];
    if (nearest === "heart") {
      this.heartValves = [0, 0, 0];
      this.heartPulses = [];
    }
    if (nearest === "feet") this.feetProgress = 0;
    this.ui.toast(`Puzzle ${nearest.toUpperCase()} reiniciado.`);
  }

  public damageArenaCover(severity: number): void {
  ...
  public revealBodyCard(nose: Mesh): void {
  ...
  private createGlobalMapBoundaries(): void {
    ...
    const wallHeight = 36;
    const thickness = 4;
    const createBoundary = (name: string, position: Vector3, width: number, depth: number): void => {
  ...
  private createDoorFrame(
    ...
    const make = (part: string, size: { width: number; height: number; depth: number }, local: Vector3, collisions = true): Mesh => {
    ...
    if (upperHeight > 0.08) make("wall-infill-upper", { width: Math.max(openingSpan, width + jamb * 2), height: upperHeight, depth: frameDepth }, new Vector3(0, height / 2 + upperHeight / 2, 0));
    if (visualRecess) {
  ...
  private setDoorwayBlocked(blocker: Mesh | undefined, blocked: boolean): void {
    if (!blocker) return;
    blocker.setEnabled(blocked);
    blocker.checkCollisions = blocked;
  }

  public closeSideEntranceBehindPlayer(): void {
    this.sideDoor.position.set(20, 2.25, 37.75);
    this.sideDoor.checkCollisions = true;
    this.setDoorwayBlocked(this.sideDoorBlocker, true);
  }
  ...
  private createFence(): void {
    ...
    for (let x = fenceMinX; x <= fenceMaxX + 0.01; x += 1.5) {
      if (x > openingMinX && x < openingMaxX) continue;
    ...
    for (const [name, x] of [["left", fenceMinX], ["right", fenceMaxX]] as const) {
    ...
    for (let y = 0.4; y < 4.3; y += 0.35) {
      for (const segment of fenceSegments) {
        const rail = MeshBuilder.CreateBox(
          `fence-wire-${segment.id}-${y.toFixed(2)}`,
          { width: segment.width, height: 0.025, depth: 0.035 },
          this.scene
        );
        rail.parent = root;
  ...
  public flashRandomCarHeadlights(durationMs = 1800): void {
    if (this.carHeadlightSets.length === 0) return;
    ...
    setEnabled(true);
    window.setTimeout(() => {
      if (token !== this.carHeadlightFlashToken) return;
      setEnabled(false);
    }, 120);
    window.setTimeout(() => {
      if (token !== this.carHeadlightFlashToken) return;
  ...
  private createCars(): void {
    ...
    positions.forEach((position, index) => {
      ...
      const addBodyPart = (name: string, size: { width: number; height: number; depth: number }, position: Vector3, material = carPaint): Mesh => {
      ...
      for (const side of [-1, 1]) {
        ...
        const light = new SpotLight(
          `car-headlight-beam-${index}-${side}`,
          new Vector3(side * 1.08, 1.02, -3.76),
      ...
      const interactionZone = MeshBuilder.CreateBox(
        `car-trunk-interaction-${index}`,
        { width: 4.2, height: 2.6, depth: 2.8 },
        this.scene
      );
      ...
      const showTrunkText = (title: string, bodyText: string): void => {
...
Pressione E novamente ou Esc para fechar.`);
      ...
      this.interaction.register(interactionZone, {
        prompt: () => {
          if (!this.openedCarTrunks.has(index)) return "[E] ABRIR O PORTA-MALAS";
          if (index === this.fuelCarIndex && !this.fuelTaken) return "[E] PEGAR O COMBUSTÍVEL";
        ...
        onInteract: () => {
          ...
          showTrunkText(
            "PORTA-MALAS",
            alreadyEmpty ? "Só restaram marcas do galão e o cheiro de gasolina." : (emptyTrunkDescriptions[index] ?? "Não há nada útil aqui.")
          );
        }
      });
    });
  ...
  private createMaintenanceShed(): void {
  ...
  private createFacilityShell(): void {
  ...
  private createContinuousFacilityFloor(): void {
    ...
    const makeOuterWall = (name: string, position: Vector3, width: number, depth: number): void => {
  ...
  private createHorizontalConnector(
    name: string,
    center: Vector3,
    width: number,
    depth: number,
  ...
  private createLobby(): void {
    ...
    let movedOnce = false;
    window.setInterval(() => {
      if (!this.enteredWonderWorld || movedOnce) return;
  ...
  private spawnGiftShopPlushWave(): void {
    const positions = [
  ...
  private createMaintenanceAndPower(): void {
    ...
    const createLockerPart = (name: string, size: { width: number; height: number; depth: number }, position: Vector3, material = lockerMaterial): Mesh => {
  ...
  private createHandsPuzzle(): void {
    ...
    for (let i = 0; i < 4; i += 1) {
      ...
      this.interaction.register(palm, {
        ...
        onInteract: () => {
          ...
          if (this.handsAngles.every((value, index) => value === target[index])) this.solvePuzzle("hands");
        }
      });
    }
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      this.interaction.register(mirror, {
        ...
        onInteract: () => {
          ...
          this.updateEyeBeams();
          if (this.eyesAngles.every((value, index) => value === targets[index])) this.solvePuzzle("eyes");
        }
      });
    }
    for (let i = 0; i < 3; i += 1) {
  ...
  private createHeartPuzzle(): void {
    ...
    this.interaction.register(pump, {
      ...
      onInteract: () => {
        ...
        if (this.heartPulses.length > 3) this.heartPulses.shift();
      ...
      this.interaction.register(panel, {
        ...
        onInteract: () => {
          ...
          if (index === this.feetSequence[this.feetProgress]) {
            ...
            if (this.feetProgress >= this.feetSequence.length) this.solvePuzzle("feet");
          } else {
            this.feetProgress = 0;
            this.audio.impact(0.45);
  ...
  private createAuditorium(): void {
    ...
    for (let index = 0; index < 32; index += 1) {
      ...
      if (entranceDistance < 0.2) continue;
      const radius = 18.6;
  ...
  private createUndergroundTransition(): void {
  ...
  private createSurvivalLoot(): void {
    this.createLootContainer(
      "exterior-emergency-chest",
      new Vector3(-24.2, 0.05, 21.2),
      "chest",
      "Pé de cabra e ferramentas de emergência.",
      () => {
        if (!this.inventory.has("crowbar")) this.inventory.add(ITEM_CATALOG.crowbar!);
        this.ui.toast("Pé de cabra equipado. Clique esquerdo ataca; botão direito aplica um golpe pesado.", 3600);
      },
      -Math.PI / 2
    );
    this.createLootContainer(
      "lobby-security-cabinet",
      new Vector3(-9.4, 0.05, 51.2),
      "cabinet",
      "Um colete acolchoado de manutenção ainda está utilizável.",
      () => {
        if (!this.inventory.has("protectiveVest")) this.inventory.add(ITEM_CATALOG.protectiveVest!);
    ...
    this.createLootContainer(
      "gift-shop-fuel-chest",
      new Vector3(-23, 0.05, 69.2),
      "chest",
      "Frascos de combustível de limpeza e um pavio seco.",
      () => {
    ...
    this.createLootContainer(
      "maintenance-medical-cabinet",
      new Vector3(-13.2, 0.05, 91.2),
      "cabinet",
      "Curativos, antisséptico e uma placa rígida para proteção.",
      () => {
        if (!this.inventory.has("medkit")) this.inventory.add(ITEM_CATALOG.medkit!);
        this.player.heal(55);
    ...
    this.createLootContainer(
      "puzzle-hall-supply-chest",
      new Vector3(11.2, 0.05, 120.4),
      "chest",
      "Um sinalizador industrial, combustível e tecido resistente.",
      () => {
        if (!this.inventory.has("flare")) this.inventory.add(ITEM_CATALOG.flare!);
    ...
    this.createLootContainer(
      "auditorium-backstage-chest",
      new Vector3(10.5, 0.05, 158.6),
      "chest",
      "Equipamento de palco, combustível e um kit de emergência.",
      () => {
    ...
    this.createLootContainer(
      "chapter2-modeling-chest",
      new Vector3(13.5, 0.05, 252),
      "chest",
      "Combustível industrial, placas de proteção e ferramentas de modelagem.",
      () => { this.fire.addFuel(38); this.player.addArmor(22); this.ui.toast("Suprimentos do departamento de modelagem coletados.", 2200); },
      Math.PI / 2
    );
    this.createLootContainer(
      "chapter2-machine-cabinet",
      new Vector3(-12.5, 0.05, 350),
      "cabinet",
      "Um kit médico lacrado e um pequeno reservatório de combustível.",
      () => { this.player.heal(45); this.fire.addFuel(25); this.ui.toast("Kit médico usado e combustível armazenado.", 2200); },
      -Math.PI / 2
    );
    this.createLootContainer(
      "chapter3-daniel-chest",
      new Vector3(5.8, 0.05, 758.5),
      "chest",
      "Peças de equipamento deixadas por Daniel: proteção, gaze e combustível.",
      () => { this.player.heal(38); this.player.addArmor(35); this.fire.addFuel(24); this.ui.toast("Equipamento de Daniel recuperado.", 2300); },
      Math.PI / 2
    );
    this.createLootContainer(
      "chapter3-generator-cabinet",
      new Vector3(14.5, 0.05, 830),
      "cabinet",
      "Máscara improvisada, placas rígidas e combustível de manutenção.",
      () => { this.player.addArmor(30); this.fire.addFuel(34); this.ui.toast("Proteção e combustível coletados.", 2200); },
      -Math.PI / 2
    );
    this.createLootContainer(
      "chapter4-guard-cabinet",
      new Vector3(9.5, 0.05, 1168.5),
      "cabinet",
      "Suprimentos confiscados: curativos, uma chapa protetora e combustível.",
      () => { this.player.heal(42); this.player.addArmor(28); this.fire.addFuel(20); this.ui.toast("Suprimentos confiscados recuperados.", 2200); },
      Math.PI / 2
    );
    this.createLootContainer(
      "chapter4-choice-chest",
      new Vector3(-2.8, 0.05, 1240),
      "chest",
      "Uma reserva escondida com proteção reforçada e itens médicos.",
      () => { this.player.heal(55); this.player.addArmor(42); this.ui.toast("Reserva de sobrevivência coletada.", 2200); },
      0.25
    );
    this.createLootContainer(
      "chapter5-archives-cabinet",
      new Vector3(12.5, 0.05, 1570),
      "cabinet",
      "Equipamento de evacuação, combustível e uma proteção leve.",
      () => { this.fire.addFuel(40); this.player.addArmor(25); this.ui.toast("Equipamento de evacuação coletado.", 2200); },
      Math.PI / 2
    );
    this.createLootContainer(
      "chapter5-bridge-chest",
      new Vector3(0, 8.06, 1951),
      "chest",
      "O último kit de emergência: combustível, curativos e placas de proteção.",
      () => { this.fire.addFuel(60); this.player.heal(70); this.player.addArmor(48); this.ui.toast("Kit final de emergência coletado.", 2600); },
      0
    );
  }
  ...
  private createLootContainer(
    ...
    const parts: Mesh[] = [];
    const makePart = (name: string, size: { width: number; height: number; depth: number }, local: Vector3, material = shellMaterial): Mesh => {
  ...
  private createCheckpointStations(): void {
    const stations: Array<[string, Vector3, Vector3, string]> = [
  ...
  public getSurvivalCheckpointPosition(id: string): Vector3 | null {
    return this.survivalCheckpointPositions.get(id)?.clone() ?? null;
  }

  private nearestUnsolvedPuzzleCheckpoint(): Vector3 {
  ...
  private solvePuzzle(id: string): void {
    if (this.solvedPuzzles.has(id)) return;
  ...
  private openAuditorium(): void {
    if (this.auditoriumOpened) return;
  ...
  private enableFacilityLights(enabled: boolean): void {
    this.scene.lights.filter((light: { name: string }) => light.name.startsWith("facility-light")).forEach((light: { setEnabled: (value: boolean) => void }) => light.setEnabled(enabled));
  }

  private updateEyeBeams(): void {
    this.audio.objective();
    const total = this.eyesAngles.reduce((sum, value) => sum + value, 0);
    const light = this.scene.getLightByName("facility-light-eyes");
    if (light) light.intensity = 0.6 + total * 0.22;
  }
  ...
  private createRoomShell(
    ...
    depth: number,
    height: number,
    floorKind: "concrete" | "metal" | "tile" | "wood",
    roof: boolean,
    openings: Array<"north" | "south" | "east" | "west"> = ["north", "south", "east", "west"]
  ): void {
    const floor = MeshBuilder.CreateBox(`${name}-floor`, { width, height: 0.2, depth }, this.scene);
    floor.position = center.add(new Vector3(0, -0.055, 0));
    ...
    const makeWall = (wallName: string, position: Vector3, size: { width: number; height: number; depth: number }): void => {
      if (size.width <= 0.02 || size.height <= 0.02 || size.depth <= 0.02) return;
    ...
    const makeHorizontalWall = (side: "north" | "south", z: number): void => {
    ...
    const makeVerticalWall = (side: "east" | "west", x: number): void => {
      if (!openings.includes(side)) {
  ...
  private createCorridor(
    ...
    doorways: { left?: number[]; right?: number[] } = {}
  ): void {
    const floor = MeshBuilder.CreateBox(`${name}-floor`, { width, height: 0.2, depth }, this.scene);
    floor.position = center.add(new Vector3(0, -0.055, 0));
    floor.material = this.materials.floor(material, Math.floor(center.z));
    ...
    const createSegmentedSide = (side: "left" | "right", x: number, gaps: number[]): void => {
      const sorted = gaps
  ...
  private createSmallPlush(name: string, position: Vector3, scale: number, variant: number): TransformNode {
