...
import type { Chapter2SaveData } from "../systems/SaveSystem";
import type { GameSettings } from "../systems/Settings";
import { MannequinSystem, type MannequinSpawn } from "../entities/MannequinSystem";

export const CHAPTER2_ITEMS: Record<string, InventoryItem> = {
...
export interface Chapter2Callbacks {
...
type SphereState = "docked" | "arena1" | "arena2" | "arena3-forward" | "arena3-return" | "settled";
...
export class Chapter2World {
  ...
  public active = false;
  public energyCells = new Set<string>();
  public mirrorSolved = false;
  public shelfSolved = false;
  public controlRoomSolved = false;
  public machineActivated = false;
  public ruleCollapsed = false;
  public completedArenas = new Set<number>();
  public mayaEventSeen = false;
  public collectedDocuments = new Set<string>();
  ...
  private readonly callbacks: Chapter2Callbacks;
  private readonly root: TransformNode;
  private readonly mannequins: MannequinSystem;
  private readonly mirrors: MirrorRig[] = [];
  private readonly shelves: ShelfRig[] = [];
  private readonly thrownObjects: ThrownObject[] = [];
  private readonly debrisPool: DebrisPiece[] = [];
  private readonly chapterLights: PointLight[] = [];
  private readonly emergencyLights: PointLight[] = [];
  private readonly machineRedLights: PointLight[] = [];
  private readonly mirrorTargets: Vector3[] = [];
  private readonly cellMeshes: Mesh[] = [];
  private readonly machineCables = [false, false, false, false];
  private readonly machineLocks = [false, false, false];
  private readonly backupSwitches = [false, false, false];
  private readonly machineCableMeshes: Mesh[] = [];
  private readonly machineLockMeshes: Mesh[] = [];
  private machinePressure = 0;
  private machineCellsInstalled = false;
  private machineCardInserted = false;
  private machineTrackAligned = false;
  private trackSwitchState = 0;
  private readonly arena2Blockers = [false, false];
  private arena2RequiredRoute = 1;
  private arena3Pass = 0;
  private shelfPositions = [1, 1, 1];
  private mirrorAngles = [0, 0, 0, 0];
  private blackoutActive = false;
  private blackoutTimer = 11;
  private blackoutElapsed = 0;
  private blackoutDamageCooldown = 0;
  private controlRoomEntered = false;
  private ruleCollapseProgress = 0;
  private ruleMessageStage = 0;
  private openingElapsed = 0;
  private openingComplete = false;
  private corridorLessonTriggered = false;
  private modelingObjectiveTriggered = false;
  private torchDisabled = true;
  private settings: GameSettings;
  private sphere!: Mesh;
  private sphereState: SphereState = "docked";
  private sphereSpeed = 0;
  private sphereTarget = Vector3.Zero();
  private sphereDamageCooldown = 0;
  private arena3HydraulicTimer = 6.5;
  private arena3HydraulicWarning = false;
  private arena3HydraulicDamageCooldown = 0;
  private activeArena = 0;
  private crushCountThisRun = 0;
  private mayaSequenceTime = -1;
  private mayaRoot: TransformNode | null = null;
  private mayaHead: Mesh | null = null;
  private mayaOriginalPositions: number[] | null = null;
  private exitTriggered = false;
  private lastRearCue = 0;
  private mirrorRenderAccumulator = 0;
  private documentMeshes = new Map<string, Mesh>();
  private readonly observationDoors: ObservationDoor[] = [];
  private soundDeviceMarker: Mesh | null = null;
  private soundDeviceCooldown = 0;
  ...
  public constructor(
    ...
    this.mannequins = new MannequinSystem(
      scene,
      materials,
      player,
      audio,
      ui,
      fire,
      this.createMannequinSpawns(),
      (amount) => this.damagePlayer(amount),
      settings.performancePreset === "performance" ? 10 : 14
    );
  ...
  public startFromElevator(): void {
    ...
    for (const mirror of this.mirrors) mirror.texture.refreshRate = this.settings.performancePreset === "performance" ? 4 : 2;
  }
  ...
  public restore(progress: Chapter2SaveData, checkpoint: string): void {
    this.active = true;
    this.root.setEnabled(true);
    this.mannequins.setChapterEnabled(true);
    this.openingComplete = true;
    ...
    this.player.setEnabled(true);
    this.startChapterSpatialAudio();
    if (this.machineActivated) this.audio.startMachineAlarm();
    if (this.mayaEventSeen) this.audio.startMusicBoxTheme();
    for (const mirror of this.mirrors) mirror.texture.refreshRate = this.settings.performancePreset === "performance" ? 4 : 2;
  }

  public serialize(): Chapter2SaveData {
    return {
  ...
  public guideTargetForObjective(objectiveId: string): Vector3 | null {
    const player = this.player.collider.position;
    const meshPoint = (name: string, fallback: Vector3): Vector3 => {
      const mesh = this.scene.getMeshByName(name);
      if (!mesh || !mesh.isEnabled()) return fallback.clone();
      mesh.computeWorldMatrix(true);
      return mesh.getBoundingInfo().boundingBox.centerWorld.clone();
    };
    const nearest = (points: Vector3[]): Vector3 | null => {
    ...
    if (objectiveId === "recover-cells") {
      const tasks: Vector3[] = [];
      if (!this.mirrorSolved) tasks.push(meshPoint("mirror-observation-console", this.checkpoints.mirrorRoom!));
      else if (!this.energyCells.has("energyCell1")) tasks.push(meshPoint("chapter2-energy-cell-1", this.checkpoints.mirrorRoom!));
      if (!this.shelfSolved) tasks.push(meshPoint("rolling-shelf-handle-0", this.checkpoints.storage!));
      else if (!this.energyCells.has("energyCell2")) tasks.push(meshPoint("chapter2-energy-cell-2", this.checkpoints.storage!));
      ...
      } else if (!this.energyCells.has("energyCell3")) tasks.push(meshPoint("chapter2-energy-cell-3", this.checkpoints.controlRoom!));
    ...
    if (objectiveId === "activate-machine") {
      const tasks: Vector3[] = [];
      this.machineCables.forEach((active, index) => { if (!active) tasks.push(meshPoint(`machine-cable-socket-${index}`, this.checkpoints.machine!)); });
      if (!this.machineCellsInstalled) tasks.push(meshPoint("machine-cell-bank", this.checkpoints.machine!));
      if (this.machinePressure !== 2) tasks.push(meshPoint("machine-pressure-console", this.checkpoints.machine!));
      if (!this.machineCardInserted) tasks.push(meshPoint("machine-body-card-reader", this.checkpoints.machine!));
      if (!this.machineTrackAligned) tasks.push(meshPoint("machine-track-lever", this.checkpoints.machine!));
      this.machineLocks.forEach((active, index) => { if (!active) tasks.push(meshPoint(`machine-release-lock-${index}`, this.checkpoints.machine!)); });
      return nearest(tasks) ?? this.checkpoints.machine!.clone();
    }
  ...
  public destinationForCheckpoint(checkpoint: string): Vector3 {
    if (checkpoint.includes("mirror") || checkpoint.includes("cell-1")) return this.checkpoints.mirrorRoom!.clone();
    if (checkpoint.includes("storage") || checkpoint.includes("cell-2")) return this.checkpoints.storage!.clone();
    if (checkpoint.includes("control") || checkpoint.includes("cell-3")) return this.checkpoints.controlRoom!.clone();
    if (checkpoint.includes("three-cells") || checkpoint.includes("machine") || checkpoint.includes("rule-collapsed")) return this.checkpoints.machine!.clone();
    if (checkpoint.includes("arena-1")) return this.checkpoints.arena1!.clone();
    if (checkpoint.includes("arena-2")) return this.checkpoints.arena2!.clone();
    if (checkpoint.includes("arena-3")) return this.checkpoints.arena3!.clone();
    if (checkpoint.includes("maya")) return this.checkpoints.maya!.clone();
    if (checkpoint.includes("chapter3")) return this.checkpoints.chapter3!.clone();
    if (checkpoint.includes("elevator-arrival")) return this.checkpoints.mannequinCorridor!.clone();
  ...
  public update(deltaSeconds: number): void {
    ...
    if (this.mirrorRenderAccumulator > 0.8) {
      ...
      for (const mirror of this.mirrors) mirror.texture.renderList = renderList;
    }
  ...
  public applySettings(settings: GameSettings): void {
    ...
    for (const mirror of this.mirrors) mirror.texture.refreshRate = settings.performancePreset === "performance" ? 4 : 2;
  }
  ...
  public handlePrimaryAttack(charged: boolean): boolean {
    if (!this.active || !this.inventory.has("crowbar") || !this.openingComplete) return false;
    if (this.meleeCooldown > 0) return true;
    ...
    const hit = this.mannequins.attackNearest(
  ...
  public resetEnemiesForPlayerRespawn(): void {
    this.mannequins.resetForPlayerRespawn();
  }

  public throwNoiseObject(): boolean {
    if (!this.active || !this.openingComplete || this.mayaSequenceTime >= 0) return false;
  ...
  public deploySoundDevice(): boolean {
    if (!this.active || !this.openingComplete) return false;
  ...
  public closeNearestRearDoor(): boolean {
  ...
  public forceBlackout(): void {
    this.controlRoomEntered = true;
    this.blackoutTimer = 0;
  }

  public grantEnergyCells(): void {
    [1, 2, 3].forEach((index) => {
  ...
  public startArena(index: number): void {
    if (!this.machineActivated) this.activateMachine(true);
  ...
  public toggleMannequinMovement(): boolean {
  ...
  public toggleObservationVisualization(): boolean {
    return this.mannequins.toggleObservationVisualization();
  }

  public toggleOcclusionVisualization(): boolean {
    return this.mannequins.toggleOcclusionVisualization();
  }

  public setMannequinCount(count: number): void {
  ...
  private debugMovementEnabled = true;

  private startChapterSpatialAudio(): void {
  ...
  private createObservationDoors(): void {
  ...
  private createObservationDoor(name: string, position: Vector3, width: number, height: number, rotationY: number): void {
    ...
    const rig: ObservationDoor = { mesh: door, blocker, open: true, closedY: position.y, openY: position.y + height + 0.5 };
  ...
  private updateObservationDoors(deltaSeconds: number): void {
  ...
  private createObservationDoorFrame(name: string, position: Vector3, width: number, height: number, rotationY: number, openingSpan = width, wallHeight = height + 0.6): Mesh {
    ...
    if (upperHeight > 0.08) make("bulkhead-upper", { width: Math.max(openingSpan, width + jamb * 2), height: upperHeight, depth: 0.7 }, new Vector3(0, height / 2 + upperHeight / 2, 0));
  ...
  private createElevatorSequenceSet(): void {
  ...
  private createMannequinCorridor(): void {
  ...
  private createModelingDepartment(): void {
  ...
  private createStorageWarehouse(): void {
  ...
  private createControlRoom(): void {
  ...
  private createCentralMachine(): void {
    ...
    for (let cableIndex = 0; cableIndex < 4; cableIndex += 1) {
      ...
      this.interaction.register(socket, {
        ...
        onInteract: () => {
          if (this.machineActivated || this.machineCables[cableIndex]) return;
    ...
    this.interaction.register(pressureConsole, {
      ...
      onInteract: () => {
        if (this.machineActivated) return;
        this.machinePressure = (this.machinePressure + 1) % 4;
        this.audio.hydraulicPulse(this.machinePressure / 3);
        if (this.machinePressure === 2) this.ui.toast("Os três manômetros entram na faixa verde.");
        this.checkMachineReadiness();
      }
    });
    ...
    this.interaction.register(cardReader, {
      ...
      onInteract: () => {
        if (this.machineCardInserted) return;
    ...
    this.interaction.register(trackLever, {
      ...
      onInteract: () => {
        if (this.machineTrackAligned) return;
    ...
    for (let lockIndex = 0; lockIndex < 3; lockIndex += 1) {
      ...
      this.interaction.register(lock, {
        ...
        onInteract: () => {
          if (this.machineActivated || this.machineLocks[lockIndex]) return;
  ...
  private createSphereArenas(): void {
    this.createArenaOne();
    this.createArenaTwo();
    this.createArenaThree();
  }

  private createArenaOne(): void {
  ...
  private createArenaTwo(): void {
    ...
    [-9, 9].forEach((x, routeIndex) => {
      ...
      this.interaction.register(hydraulic, {
        ...
        onInteract: () => {
          if (this.arena2Blockers[routeIndex]) return;
  ...
  private createArenaThree(): void {
    ...
    this.interaction.register(returnWinch, {
      ...
      onInteract: () => {
        if (this.arena3Pass !== 1 || this.sphereState !== "settled") return;
  ...
  private createMayaAndExitRoute(): void {
  ...
  private createFunctionalMirror(index: number, position: Vector3, baseAngle: number): void {
    ...
    const texture = (surface.material as StandardMaterial).reflectionTexture as MirrorTexture;
    const rig: MirrorRig = { id: index, pivot, surface, texture, angleIndex: 0, baseAngle, width: 4.1, height: 3.7, damage: 0.12 + index * 0.08 };
  ...
  private createMirrorMaterial(index: number, position: Vector3, angle: number): StandardMaterial {
  ...
  private createMirrorDamageMaterial(index: number): StandardMaterial {
  ...
  private updateMirrorPlane(rig: MirrorRig): void {
  ...
  private mirrorNormal(angle: number): Vector3 {
    return new Vector3(Math.sin(angle), 0, -Math.cos(angle)).normalize();
  }

  private evaluateMirrorPuzzle(fromConsole: boolean): void {
    if (this.mirrorSolved) return;
    const target = [0, 0, 3, 1];
    const aligned = this.mirrorAngles.every((angle, index) => angle === target[index]);
    if (!aligned) {
      if (fromConsole) this.ui.toast("As reflexões não cobrem todas as zonas cegas.");
      return;
    }
    if (!fromConsole) {
  ...
  private isObservedThroughMirror(mannequinPosition: Vector3): boolean {
    if (!this.active || this.blackoutActive) return false;
    ...
    for (const mirror of this.mirrors) {
      ...
      if (mirrorDistance < 0.1 || mirrorDistance > 22) continue;
      if (Vector3.Dot(cameraForward, toMirror.scale(1 / mirrorDistance)) < 0.4) continue;
      ...
      if (Math.abs(localX) > mirror.width * 0.5 * (1 - mirror.damage)) continue;
      if (Math.abs(local.y) > mirror.height * 0.5 * (1 - mirror.damage * 0.6)) continue;
      if (this.rayBlocked(cameraPosition, intersection, mirror.surface)) continue;
      if (this.rayBlocked(intersection.add(normal.scale(0.08)), mannequinPosition.add(new Vector3(0, 1.1, 0)), mirror.surface)) continue;
      return true;
    }
    return false;
  }

  private rayBlocked(from: Vector3, to: Vector3, ignored: AbstractMesh): boolean {
    const offset = to.subtract(from);
    const distance = offset.length();
    if (distance < 0.1) return false;
    const pick = this.scene.pickWithRay(new Ray(from, offset.scale(1 / distance), distance), (mesh: AbstractMesh) => {
      if (!mesh.isPickable || !mesh.isVisible || !mesh.isEnabled()) return false;
      if (mesh === ignored || mesh === this.player.collider || mesh.metadata?.mannequinVisual) return false;
      return true;
    });
    return Boolean(pick?.hit && pick.distance < distance - 0.25);
  }

  private createEnergyCell(index: number, position: Vector3): Mesh {
  ...
  private collectEnergyCell(index: number): void {
    const id = `energyCell${index}`;
    if (this.energyCells.has(id)) return;
    const item = CHAPTER2_ITEMS[id];
    if (!item) return;
    this.energyCells.add(id);
  ...
  private moveShelf(index: number): void {
    const shelf = this.shelves[index];
    if (!shelf || this.shelfSolved) return;
  ...
  private evaluateShelfPuzzle(): void {
    if (this.shelfSolved) return;
    const target = [0, 2, 1];
    if (!this.shelfPositions.every((value, index) => value === target[index])) return;
    this.shelfSolved = true;
    const door = this.scene.getMeshByName("storage-cell-cage-door");
    if (door) {
      door.position.y = 5.1;
  ...
  private activateBackupSwitch(index: number, mesh: Mesh): void {
    if (this.backupSwitches[index]) return;
  ...
  private installEnergyCells(bank: Mesh): void {
    if (this.machineActivated) return;
    if (this.energyCells.size < 3) {
      this.ui.toast(`Faltam ${3 - this.energyCells.size} células.`);
      return;
    }
    if (this.machineCellsInstalled) return;
  ...
  private createInstalledCellVisuals(bank: Mesh): void {
    [1, 2, 3].forEach((index) => {
      if (this.scene.getMeshByName(`machine-installed-cell-${index}`)) return;
  ...
  private checkMachineReadiness(): void {
    if (this.machineActivated) return;
  ...
  private activateMachine(debugBypass: boolean): void {
    if (this.machineActivated) return;
    if (debugBypass) {
      ...
      const bank = this.scene.getMeshByName("machine-cell-bank");
      this.machineCellsInstalled = true;
      if (bank) bank.metadata = { ...(bank.metadata ?? {}), cellsInstalled: true };
    }
  ...
  private animateTrackAlignment(): void {
    for (let index = -2; index <= 2; index += 1) {
      const rail = this.scene.getMeshByName(`central-track-${index}`);
      if (rail) rail.position.x = index * 2.2;
    }
  }

  private createLureButton(name: string, position: Vector3, lurePosition: Vector3, strength: number): Mesh {
  ...
  private createReleaseConsole(name: string, position: Vector3, arena: number): Mesh {
  ...
  private releaseSphere(arena: number): void {
    if (!this.machineActivated) {
      this.ui.toast("A esfera ainda está presa à máquina central.");
      return;
    }
    if (this.completedArenas.has(arena)) return;
  ...
  private prepareArena(index: number): void {
  ...
  private updateOpening(deltaSeconds: number): void {
    ...
    if (indicator?.material && Math.floor(this.openingElapsed * 1.1) !== Math.floor((this.openingElapsed - deltaSeconds) * 1.1)) {
      ...
      if (floor === 3 || floor === 5) this.audio.impact(0.45);
    }
    if (this.openingElapsed > 2.2 && this.openingElapsed < 5.8) {
      ...
      if (Math.random() < deltaSeconds * 1.1) this.ui.showSoundCaption("o elevador range entre os níveis", 900);
    }
  ...
  private updateShelves(deltaSeconds: number): void {
    this.shelves.forEach((shelf, index) => {
      ...
      if (Math.random() < deltaSeconds * 2) this.audio.mannequinJoint(shelf.root.position, 0.2);
      this.shelfPositions[index] = shelf.targetState;
    });
  }

  private updateBlackout(deltaSeconds: number): void {
    if (!this.controlRoomEntered || this.controlRoomSolved) return;
  ...
  private updateRuleCollapse(deltaSeconds: number): void {
    if (!this.machineActivated || this.ruleCollapsed) return;
  ...
  private updateSphere(deltaSeconds: number): void {
    if (!this.sphere || this.sphereState === "docked" || this.sphereState === "settled") return;
    ...
    if (this.activeArena === 3 && Math.random() < deltaSeconds * 1.2) this.dropArenaDebris();
  }

  private finishSphereRun(): void {
  ...
  private updateThrownObjects(deltaSeconds: number): void {
    for (const object of this.thrownObjects) {
      ...
      const pick = distance > 0.001
        ? this.scene.pickWithRay(new Ray(object.mesh.position, direction.scale(1 / distance), distance + 0.15), (mesh: AbstractMesh) => mesh.checkCollisions && mesh !== this.player.collider)
        : null;
      ...
      if (debris.mesh.position.y < 0.12) {
        ...
        if (debris.hazardous) {
          ...
          const horizontalDistance = Math.hypot(
            debris.mesh.position.x - playerPosition.x,
            debris.mesh.position.z - playerPosition.z
          );
  ...
  private updateMayaSequence(deltaSeconds: number): void {
    if (this.mayaSequenceTime < 0) return;
    this.mayaSequenceTime += deltaSeconds;
    const hand = this.scene.getMeshByName("maya-ankle-hand");
    if (hand) hand.setEnabled(true);
    if (!this.mayaRoot) this.createMayaMannequin();
    if (this.mayaRoot) {
  ...
  private updateAreaTriggers(): void {
  ...
  private createMayaMannequin(): void {
  ...
  private applyMayaFace(visible: boolean): void {
    if (!this.mayaHead) return;
    if (!visible) {
      this.mayaHead.material = this.materials.get("plastic", 1);
      if (this.mayaOriginalPositions) this.mayaHead.updateVerticesData("position", [...this.mayaOriginalPositions], true, false);
      return;
    }
    ...
    if (positions) {
      for (let index = 0; index < positions.length; index += 3) {
        ...
        if (z < -0.18 && y > -0.12 && Math.abs(x) < 0.18) positions[index + 2] = z - 0.055 * (1 - Math.abs(x) / 0.18);
        if (z < -0.12 && y < -0.08 && y > -0.28) positions[index + 1] = y - 0.035;
      }
      this.mayaHead.updateVerticesData("position", positions, true, false);
    }
  }

  private openArenaExit(): void {
    const panels = this.scene.meshes.filter((mesh: AbstractMesh) => mesh.name.startsWith("arena3-hanging-panel-"));
    panels.slice(0, 6).forEach((panel: AbstractMesh, index: number) => {
  ...
  private setChapterLighting(level: number): void {
    this.chapterLights.forEach((light, index) => light.intensity = level * (index % 3 === 0 ? 2.5 : 1.8));
  }

  private updateArenaThreeHazard(deltaSeconds: number): void {
    ...
    if (this.arena3HydraulicTimer > 0) return;
  ...
  private isOnArenaThreeSafePlatform(): boolean {
    const position = this.player.collider.position;
    return [-14, 0, 14].some((x) =>
      Math.abs(position.x - x) <= 2.75 && Math.abs(position.z - 475) <= 2.75
    );
  }

  private dropArenaDebris(): void {
  ...
  private spawnCrushDebris(position: Vector3, count: number): void {
    let spawned = 0;
    for (const debris of this.debrisPool) {
      if (debris.active) continue;
      debris.active = true;
  ...
  private createThrownObjectPool(): void {
    for (let index = 0; index < 8; index += 1) {
  ...
  private applyRestoredVisualState(): void {
    ...
    this.machineCables.forEach((active, index) => {
      ...
      if (cable) cable.scaling.y = active ? 1 : 0.38;
    });
    this.machineLocks.forEach((active, index) => {
      const lock = this.machineLockMeshes[index];
      if (lock) lock.rotation.x = active ? -0.8 : 0;
    });
    if (this.machineCellsInstalled) {
      const bank = this.scene.getMeshByName("machine-cell-bank");
      if (bank) {
        bank.metadata = { ...(bank.metadata ?? {}), cellsInstalled: true };
        this.createInstalledCellVisuals(bank as Mesh);
      }
    }
    ...
    if (this.machineTrackAligned) this.animateTrackAlignment();
    if (this.machineCardInserted) {
      const reader = this.scene.getMeshByName("machine-body-card-reader");
      if (reader) reader.material = this.materials.emissive("card-reader-active", new Color3(0.55, 0.07, 0.035), 0.7);
    }
    if (this.machineActivated) {
      this.machineRedLights.forEach((light) => light.intensity = 4.5);
      if (this.completedArenas.has(3)) this.sphere.position = new Vector3(0, 3.45, 463);
      else if (this.completedArenas.has(2)) this.sphere.position = new Vector3(0, 3.45, 462);
      else if (this.completedArenas.has(1)) this.sphere.position = new Vector3(0, 3.45, 416);
      else this.sphere.position = new Vector3(0, 3.45, 354);
  ...
  private createRoom(
    ...
    height: number,
    material: "concrete" | "metal" | "tile",
    doorways: Array<{ side: "north" | "south" | "east" | "west"; offset: number; width: number }> = []
  ): void {
    const room = new TransformNode(name, this.scene);
    room.parent = this.root;
    room.position.copyFrom(center);
    ...
    const buildSide = (side: "north" | "south" | "east" | "west"): void => {
      ...
      for (const opening of relevant) {
        if (opening.start > cursor + 0.02) spans.push({ start: cursor, end: opening.start });
        cursor = Math.max(cursor, opening.end);
      }
      if (cursor < total / 2 - 0.02) spans.push({ start: cursor, end: total / 2 });
      if (relevant.length === 0) spans.splice(0, spans.length, { start: -total / 2, end: total / 2 });
  ...
  private createTextSign(name: string, text: string, position: Vector3, width: number, height: number, color: Color3): Mesh {
  ...
  private updateTextSign(sign: AbstractMesh, text: string): void {
    const material = sign.material as StandardMaterial | null;
  ...
  private createMeasurementDiagram(name: string, position: Vector3, rotationY: number): void {
  ...
  private createMannequinSpawns(): MannequinSpawn[] {
    const spawns: MannequinSpawn[] = [];
    const addRows = (centerX: number, startZ: number, rows: number, columns: number, spacingX: number, spacingZ: number, groupOffset: number): void => {
