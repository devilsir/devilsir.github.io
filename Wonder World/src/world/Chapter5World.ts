...
import type { Chapter5SaveData } from "../systems/SaveSystem";
import { NoiseSystem } from "../systems/NoiseSystem";
import { JesseSystem, type JesseTraversalNode } from "../entities/JesseSystem";
import { NoahCompanion, type NoahCommand } from "../entities/NoahCompanion";

export const CHAPTER5_ITEMS: Record<string, InventoryItem> = {
...
interface ArchiveRecord {
  ...
  title: string;
  body: string;
  category: "body" | "movement" | "fear" | "identity" | "permanent" | "protagonist" | "government";
  mesh: Mesh;
}

interface ProofStation {
  id: string;
  used: boolean;
  mesh: Mesh;
}
...
export class Chapter5World {
  ...
  public active = false;
  public phase: Chapter5Phase = "approach";
  public chapterComplete = false;
  public trueEndingDiscovered = false;
  ...
  private readonly callbacks: Chapter5Callbacks;
  private readonly root: TransformNode;
  private readonly noah: NoahCompanion;
  private readonly noise: NoiseSystem;
  private readonly jesse: JesseSystem;
  private readonly doors: DoorRig[] = [];
  private readonly proofStations: ProofStation[] = [];
  private readonly archiveRecords: ArchiveRecord[] = [];
  private readonly documents = new Set<string>();
  private readonly recordings = new Set<string>();
  private readonly rescuedDocuments = new Set<string>();
  private readonly flashbacksSeen = new Set<string>();
  private readonly proofAnswers = new Map<string, string>();
  private readonly bridgeSections: BridgeSection[] = [];
  private readonly mannequins: SurvivingMannequin[] = [];
  private readonly mirrorTextures: MirrorTexture[] = [];
  private proofScore = 0;
  private proofContradictions = 0;
  private proofComplete = false;
  private archiveDataAccessed = false;
  private experimentRevelationSeen = false;
  private protagonistPastRevealed = false;
  private collapseStarted = false;
  private escapeSegment = 0;
  private noahReaderReady = false;
  private playerReaderReady = false;
  private readerClock = 0;
  private jesseChaseActive = false;
  private jesseCheckpoint = 0;
  private postCreditsSeen = false;
  private creditsSeen = false;
  private finalDialogueSeen = false;
  private finalSequenceLocked = false;
  private mimicAttackCount = 0;
  private deathCount = 0;
  private completionSeconds = 0;
  private campaignStartTimestamp = Date.now();
  private optionalRescueClock = 0;
  private smokeClock = 0;
  private mirrorRefreshClock = 0;
  private authorityClock = 0;
  private finalNose!: Mesh;
  private proofBarrier!: Mesh;
  private archiveCoreConsole!: Mesh;
  private cooperativeDoor!: Mesh;
  private noahHeavyHandle!: Mesh;
  private bodyBeam!: Mesh;
  private blastDoor!: Mesh;
  private playerReader!: Mesh;
  private noahReader!: Mesh;
  private exitDoorLeft!: Mesh;
  private exitDoorRight!: Mesh;
  private exitDoorBlocker!: Mesh;
  private childhoodProjector!: Mesh;
  private childhoodToken!: Mesh;
  private archiveFireStarted = false;
  private mannequinLightOn = true;
  private mannequinLightClock = 0;
  ...
  public constructor(
    ...
    this.jesse = new JesseSystem(scene, materials, player, audio, ui, this.noise, {
      ...
      onStateChanged: (state) => {
        if (state === "chasing") this.ui.showSoundCaption("a melodia acelera atrás de vocês", 1800);
      },
  ...
  public startFromTransition(mimicAttackCount: number): void {
    this.active = true;
  ...
  public restore(save: Chapter5SaveData, checkpoint: string): void {
    this.active = true;
    this.root.setEnabled(true);
    this.phase = save.phase as Chapter5Phase;
    ...
    this.player.setEnabled(true);
    if (save.jesseChaseActive) this.startFinalJesseChase(Math.max(0, save.jesseCheckpoint));
    if (save.phase === "credits") this.beginCredits();
    else if (save.phase === "post-credits") this.beginPostCredits();
    else if (save.phase === "complete") this.callbacks.onChapterComplete();
  }

  public serialize(): Chapter5SaveData {
  ...
  public update(deltaSeconds: number): void {
    ...
    if (this.jesseChaseActive) this.jesse.update(deltaSeconds);
  ...
  public destinationForCheckpoint(checkpoint: string): Vector3 {
    if (checkpoint.startsWith("chapter5-jesse")) return this.checkpoints.finalJesse.clone();
    if (checkpoint === "chapter5-proof") return this.checkpoints.proof.clone();
    if (checkpoint === "chapter5-archives") return this.checkpoints.employeeArchives.clone();
    if (checkpoint === "chapter5-memory") return this.checkpoints.memoryLab.clone();
    if (checkpoint === "chapter5-collapse") return this.checkpoints.archiveCore.clone();
    if (checkpoint === "chapter5-body-return") return this.checkpoints.burnedAuditorium.clone();
    if (checkpoint === "chapter5-mannequin-return") return this.checkpoints.mannequinTransit.clone();
    if (checkpoint === "chapter5-burning-archives") return this.checkpoints.burningArchives.clone();
    if (checkpoint === "chapter5-bridge") return this.checkpoints.bridge.clone();
    if (checkpoint === "chapter5-exit") return this.checkpoints.finalExit.clone();
    if (checkpoint === "chapter5-authorities" || checkpoint === "true-ending") return this.checkpoints.authorities.clone();
    return this.checkpoints.entrance.clone();
  }

  public commandNoah(command: NoahCommand): void {
  ...
  public handleContextAction(): boolean {
    if (!this.active || !this.proofComplete) return false;
    ...
    this.ui.showTacticalActions([
      ...
      if (["follow", "wait", "move", "distract"].includes(action)) this.commandNoah(action as NoahCommand);
    });
    return true;
  }
  ...
  public handlePrimaryAttack(charged: boolean): boolean {
    ...
    const pick = this.scene.pickWithRay(ray, (mesh: AbstractMesh) => mesh.isPickable && mesh.isVisible && mesh.isEnabled());
    if (pick?.hit && pick.pickedMesh) {
      ...
      if (pick.pickedMesh.name.includes("final-obstacle")) pick.pickedMesh.setEnabled(false);
    }
    return true;
  }

  public emitPlayerMovementNoise(intensity: number, surface: "concrete" | "tile" | "wood", sprinting: boolean, crouching: boolean): void {
    if (!this.active || !this.collapseStarted) return;
    this.noise.emit({
  ...
  public throwObject(): boolean {
    if (!this.active || !this.inventory.has("metalCan")) return false;
  ...
  public debugTeleport(destination: keyof Chapter5World["checkpoints"]): void {
    this.enableForDebug();
    this.player.teleport(this.checkpoints[destination].clone(), 0);
    if (destination === "finalJesse") this.startFinalJesseChase(0);
  }

  public debugSetNoah(command: NoahCommand): void {
    this.enableForDebug();
    this.commandNoah(command);
  }

  public debugTriggerTrueEnding(): void {
    this.enableForDebug();
    this.proofComplete = true;
  ...
  public getNoah(): NoahCompanion {
    return this.noah;
  }

  private createConnectedArchiveComplex(): void {
  ...
  private createProofOfLifeSequence(): void {
    ...
    this.interaction.register(mirror, {
      ...
      onInteract: () => this.runProofQuestion(
        "mirror",
        "Noah levanta a mão esquerda. O reflexo parece normal — e o ferimento limita o braço direito.",
        [
    ...
    this.interaction.register(recorder, {
      ...
      onInteract: () => this.runProofQuestion(
        "recording",
        "Noah pede para ouvir a última gravação de Daniel. Ele interrompe antes da frase final.",
        [
    ...
    this.interaction.register(token, {
      ...
      onInteract: () => this.runProofQuestion(
        "parking-token",
        "Noah olha para a ficha azul e pergunta quem ficou do lado de fora quando os outros entraram.",
        [
  ...
  private createArchiveRecords(): void {
    const definitions: Omit<ArchiveRecord, "mesh">[] = [
      {
        id: "symbolic-program",
    ...
    definitions.forEach((definition, index) => {
      ...
      const record: ArchiveRecord = { ...definition, mesh };
  ...
  private createMemoryTransferLab(): void {
  ...
  private createCollapseRoute(): void {
  ...
  private createBurnedAuditoriumReturn(): void {
  ...
  private createMannequinReturn(): void {
  ...
  private createFinalJesseRoute(): void {
  ...
  private createBurningArchives(): void {
  ...
  private createMaintenanceBridge(): void {
  ...
  private createAuthoritiesExterior(): void {
    ...
    for (let index = 0; index < 6; index += 1) {
      const search = new SpotLight(
        `authority-searchlight-${index}`,
        new Vector3(-26 + index * 10, 8, 2050),
        new Vector3((index - 3) * 0.06, -0.35, 1),
        Math.PI / 5,
        1.4,
        this.scene
      );
  ...
  private createFunctionalMirrors(): void {
  ...
  private updateProofSequence(): void {
    ...
    if (this.phase === "proof" && this.proofStations.every((station) => station.used) && !this.proofComplete) this.completeProofOfLife();
  }

  private runProofQuestion(
    stationId: string,
    prompt: string,
    answers: { id: string; label: string; correct: boolean }[]
  ): void {
    const station = this.proofStations.find((entry) => entry.id === stationId);
    if (!station || station.used) return;
    this.player.setEnabled(false);
  ...
  private completeProofOfLife(): void {
    this.proofComplete = true;
    this.phase = "archives";
  ...
  private reactToMimicBehavior(): void {
  ...
  private readArchiveRecord(record: ArchiveRecord): void {
    if (!this.documents.has(record.id)) {
      this.documents.add(record.id);
      this.audio.pickup();
    }
    this.ui.showDocument(record.title, record.body, () => {
      if (["symbolic-program", "permanent-visitors-a", "protagonist-file"].includes(record.id)) this.noahReactToRecord(record);
      if (this.documents.size >= 4) this.objective.set("find-own-file", "ENCONTRE SEU ARQUIVO INFANTIL E ACESSE O NÚCLEO.");
    });
  }

  private noahReactToRecord(record: ArchiveRecord): void {
    if (record.id === "symbolic-program") this.ui.showSubtitle("Noah", "Corpo. Movimento. Medo. Identidade. A gente passou por cada parte como se fosse um teste.", 5600);
    if (record.id === "permanent-visitors-a") this.ui.showSubtitle("Noah", "Eles não perderam essas crianças. Eles decidiram ficar com elas.", 4800);
    if (record.id === "protagonist-file") this.ui.showSubtitle("Noah", "A ficha no chão... você não encontrou hoje. Você voltou para buscar sem saber.", 5200);
  }

  private playChildhoodFlashback(id: string): void {
  ...
  private canAccessArchiveCore(): boolean {
  ...
  private revealExperimentProgram(): void {
    if (this.archiveDataAccessed) return;
    this.archiveDataAccessed = true;
  ...
  private beginFacilityFailure(): void {
    ...
    for (const light of this.scene.lights as Array<{ intensity: number }>) light.intensity *= 0.72;
  ...
  private updateArchiveProgress(): void {
    if (!this.active || !this.proofComplete) return;
  ...
  private updateCollapse(deltaSeconds: number): void {
    if (!this.collapseStarted) return;
    ...
    if (this.phase === "mannequin-return" && z > 1810) this.startFinalJesseChase(0);
    if (this.phase === "burning-archives") {
      ...
      if (!this.archiveFireStarted) {
        ...
        for (let index = 0; index < 8; index += 1) this.fire.igniteAt(new Vector3(index % 2 ? -7 : 7, 0.4, 1882 + index * 5.4), 20, index < 4);
      }
      if (z > 1928) {
        this.phase = "bridge";
        this.noah.setFollow();
  ...
  private startFinalJesseChase(checkpoint: number): void {
    if (this.jesseChaseActive) return;
  ...
  private updateMannequinReturn(deltaSeconds: number): void {
    if (this.phase !== "mannequin-return") return;
    ...
    for (const mannequin of this.mannequins) {
      ...
      if (distance < 18 && performance.now() - mannequin.lastObserved > 260) {
        ...
        towardPlayer.y = 0;
        if (towardPlayer.length() > 2.3) mannequin.root.position.addInPlace(towardPlayer.normalize().scale(deltaSeconds * 1.9));
  ...
  private updateBridge(deltaSeconds: number): void {
    if (this.phase !== "bridge") return;
  ...
  private updateFinalReaders(deltaSeconds: number): void {
  ...
  private synchronizeFinalExit(): void {
  ...
  private finishTrueEnding(): void {
    if (this.trueEndingDiscovered) return;
  ...
  private beginCredits(): void {
    if (this.phase === "credits" && this.creditsSeen) return;
    this.phase = "credits";
  ...
  private beginPostCredits(): void {
    if (this.postCreditsSeen) return;
  ...
  private updateAuthorities(deltaSeconds: number): void {
    if (this.phase !== "authorities" && this.phase !== "credits") return;
  ...
  private updateMirrors(deltaSeconds: number): void {
    this.mirrorRefreshClock -= deltaSeconds;
    if (this.mirrorRefreshClock > 0) return;
  ...
  private handleNoahReachedTarget(tag: string): void {
  ...
  private pathObstructed(from: Vector3, to: Vector3): boolean {
    const direction = to.subtract(from);
    const distance = direction.length();
    if (distance <= 0.01) return false;
    const ray = new Ray(from, direction.scale(1 / distance), distance);
    const pick = this.scene.pickWithRay(ray, (mesh: AbstractMesh) => mesh.checkCollisions && mesh.isEnabled() && mesh.isVisible);
    return Boolean(pick?.hit && pick.distance < distance - 0.4);
  }

  private applyPersistentVisualState(): void {
    ...
    if (this.escapeSegment >= 2) this.bodyBeam.position.y = 3.1;
  ...
  private createRoom(name: string, center: Vector3, width: number, height: number, depth: number, material: "concrete" | "metal" | "tile" | "wood" | "burned", variant: number): void {
    ...
    if (!right) throw new Error(`Unable to clone room wall: ${name}`);
  ...
  private createCorridor(name: string, center: Vector3, width: number, height: number, depth: number, material: "concrete" | "metal", variant: number): void {
    this.createRoom(name, center, width, height, depth, material, variant);
  }

  private createDoor(name: string, position: Vector3, offset: Vector3, material: "metal" | "wood", variant: number): Mesh {
  ...
  private createDoorFrame(
    ...
    if (upperHeight > 0.08) make("bulkhead-upper", { width: Math.max(openingSpan, width + jamb * 2), height: upperHeight, depth: 0.82 }, new Vector3(0, height / 2 + upperHeight / 2, 0));
  ...
  private createArchiveSign(index: number): Mesh {
  ...
  private createEmergencyVehicle(index: number): TransformNode {
  ...
  private createAuthoritySilhouette(index: number): TransformNode {
