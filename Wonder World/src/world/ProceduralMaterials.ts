...
export type MaterialKind = "concrete" | "metal" | "plush" | "plastic" | "tile" | "wood" | "glass" | "burned";
...
export class ProceduralMaterials {
  ...
  public constructor(scene: Scene, textureSize = 512) {
    this.scene = scene;
    this.size = textureSize;
  }

  public get(kind: MaterialKind, variant = 0): PBRMaterial {
  ...
  public floor(kind: Exclude<MaterialKind, "glass" | "plush">, variant = 0): PBRMaterial {
  ...
  public emissive(name: string, color: Color3, intensity = 1): PBRMaterial {
  ...
  public solid(name: string, color: Color3, roughness = 0.7, metallic = 0): PBRMaterial {
  ...
  private makeTexture(kind: MaterialKind, variant: number, bump: boolean): DynamicTexture {
  ...
  private sample(
    ...
    random: () => number,
    variant: number
  ): { base: number; damage: number; wet: number } {
  ...
  private drawAuthoredDetails(
    ...
    if (kind === "metal") {
      ...
      const boltPositions: Array<[number, number]> = [[52, 52], [this.size - 52, 52], [52, this.size - 52], [this.size - 52, this.size - 52]];
  ...
  private palette(kind: MaterialKind, variant: number): Palette {
  ...
  private mixPalette(palette: Palette, base: number, damage: number, wet: number): [number, number, number] {
  ...
  private normalColor(kind: MaterialKind, u: number, v: number, variant: number): [number, number, number] {
    ...
    const heightAt = (sampleU: number, sampleV: number): number => {
  ...
  private roughness(kind: MaterialKind): number {
    return {
      concrete: 0.91,
      metal: 0.55,
      plush: 0.94,
  ...
  private crackField(u: number, v: number, variant: number): number {
  ...
  private circleMask(u: number, v: number, cx: number, cy: number, radius: number): number {
