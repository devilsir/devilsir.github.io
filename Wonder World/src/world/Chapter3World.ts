...
import type { Chapter3SaveData, MayaEncounterState } from "../systems/SaveSystem";
import { NoiseSystem, type NoiseCategory, type NoiseMaterial } from "../systems/NoiseSystem";
import { JesseSystem, type JesseTraversalNode } from "../entities/JesseSystem";
import { MimicEntity, type MimicIdentity } from "../entities/MimicEntity";

export const CHAPTER3_ITEMS: Record<string, InventoryItem> = {
...
export interface Chapter3Callbacks {
  onCheckpoint: (checkpoint: string) => void;
  onChapterComplete: () => void;
  onPlayerDamaged: () => void;
}

type Chapter3Phase = "arrival" | "jack" | "chase" | "daniel" | "generators" | "maya" | "complete";
type TraversalType = "vault" | "slide";
type MinibossState = "dormant" | "stalking" | "attacking" | "staggered" | "dead";

interface TraversalObstacle {
  mesh: Mesh;
  type: TraversalType;
  completed: boolean;
  checkpoint: number;
...
interface BreakableRig {
  mesh: Mesh;
  health: number;
  requiredCharge: boolean;
}

interface MinibossRig {
  root: TransformNode;
  collider: Mesh;
  parts: Mesh[];
  health: number;
...
export class Chapter3World {
  ...
  public active = false;
  public phase: Chapter3Phase = "arrival";
  public chaseComplete = false;
  public danielRecording = false;
  public clubOwned = false;
  public replacementTorchOwned = false;
  public accessCardObtained = false;
  public generatorStates = [false, false, false, false, false];
  public minibossDefeated = false;
  public melodySolved = false;
  public mayaEncounterState: MayaEncounterState = "not-started";
  ...
  private readonly callbacks: Chapter3Callbacks;
  private readonly root: TransformNode;
  private readonly noise: NoiseSystem;
  private readonly jesse: JesseSystem;
  private readonly mimic: MimicEntity;
  private mimicMirrorTexture: MirrorTexture | null = null;
  private settings: GameSettings;
  private readonly lights: PointLight[] = [];
  private readonly dangerLights: PointLight[] = [];
  private readonly doors: DoorRig[] = [];
  private readonly hidingSpots: HidingSpot[] = [];
  private readonly traversalObstacles: TraversalObstacle[] = [];
  private readonly thrownCans: ThrownCan[] = [];
  private readonly breakables: BreakableRig[] = [];
  private readonly generators: GeneratorRig[] = [];
  private readonly componentSlots: ComponentSlot[] = [];
  private readonly componentMeshes = new Map<string, Mesh>();
  private readonly documentMeshes = new Map<string, Mesh>();
  private readonly chaseRoute: Vector3[] = [];
  private readonly jesseNodes: JesseTraversalNode[] = [];
  private readonly cableRoute = [0, 0, 0];
  private readonly breakerStates = [false, false, false];
  private readonly gasVents = [false, false, false];
  private readonly melodyInput: number[] = [];
  private readonly correctMelody = [4, 5, 3, 1, 2, 0];
  private readonly componentOrder = ["rotor", "insulator", "brush", "fuse", "regulator", "contact"];
  private selectedComponent = "";
  private gasConcentration = 0.88;
  private gasBreath = 8;
  private torchDisabledByChapter = false;
  private jackBoxRoot!: TransformNode;
  private jackLid!: Mesh;
  private jackCrank!: TransformNode;
  private jackSequence = -1;
  private chaseCheckpoint = 0;
  private chaseStarted = false;
  private chaseElevatorClosing = false;
  private chaseElevatorTimer = 0;
  private traversalMotion: TraversalMotion | null = null;
  private hiddenSpot: HidingSpot | null = null;
  private clubCooldown = 0;
  private chargedAttackReady = false;
  private finalGeneratorLever!: Mesh;
  private mayaSequence = -1;
  private mimicAttackTriggered = false;
  private memorySequence = -1;
  private chapterExitTriggered = false;
  private noiseIndicatorWasVisible = false;
  private lightReaction = 0;
  private generator1Attempts = 0;
  private gasGeneratorRepaired = false;
  private gasEntered = false;
  private gasNoiseTimer = 1.5;
  private miniboss!: MinibossRig;
  private assemblyCheckMesh!: Mesh;
  private gasRepairConsole!: Mesh;
  private melodyPlayback!: Mesh;
  private generator5PlayerLever!: Mesh;
  private generator5MayaLever!: Mesh;
  private accessCardMesh!: Mesh;
  private danielItemsCollected = 0;
  private lastSafeCheckpoint = "chapter3Start";
  private debugForceJesse = false;
  ...
  public constructor(
    ...
    this.jesse = new JesseSystem(scene, materials, player, audio, ui, this.noise, {
      ...
      onStateChanged: (state) => {
        if (state === "investigating") this.ui.showSoundCaption("a melodia de Jesse se aproxima");
      },
  ...
  public restore(progress: Chapter3SaveData, checkpoint: string): void {
    this.active = true;
    this.root.setEnabled(true);
    this.noise.setEnabled(true);
    ...
    else if (this.mayaEncounterState !== "not-started") this.phase = "maya";
    else if (!this.chaseComplete) {
      this.phase = checkpoint.startsWith("jesse-chase-") ? "chase" : checkpoint === "jackChamber" ? "jack" : "arrival";
    } else if (!danielEquipmentReady) this.phase = "daniel";
    else this.phase = "generators";
    ...
    if (this.phase === "arrival") {
      ...
      this.objective.set(
        this.generatorStates.slice(0, 4).every(Boolean) && !this.generatorStates[4] ? "reach-generator5" : "activate-five-generators",
        this.generatorStates.slice(0, 4).every(Boolean) && !this.generatorStates[4]
      ...
      if (this.mayaEncounterState === "assisting") {
        ...
        this.objective.set(
          this.generatorStates[4] ? "watch-maya" : "synchronize-generator5",
          this.generatorStates[4] ? "OBSERVE MAYA." : "SEGURE A ALAVANCA ESQUERDA ENQUANTO MAYA MANTÉM A OUTRA."
        );
  ...
  public serialize(): Chapter3SaveData {
    return {
  ...
  public destinationForCheckpoint(checkpoint: string): Vector3 {
    ...
    if (this.checkpoints[normalized]) return this.checkpoints[normalized]!.clone();
    ...
    if (checkpoint === "daniel") return this.checkpoints.danielRoom.clone();
    if (checkpoint === "maya-reveal") return this.checkpoints.mayaChamber.clone();
  ...
  public emitPlayerMovementNoise(intensity: number, material: NoiseMaterial, sprinting: boolean, crouching: boolean): void {
    if (!this.active || this.phase === "complete") return;
    const category: NoiseCategory = crouching ? "crouch" : sprinting ? "sprint" : "footstep";
    this.emitNoise(category, intensity, material, this.player.collider.position);
  }

  public emitTorchNoise(activeUse: boolean): void {
    if (!this.active || !this.fire.torchLit) return;
  ...
  public handlePrimaryAttack(charged: boolean): boolean {
    if (!this.active || !this.clubOwned || this.phase === "chase" || this.phase === "arrival" || this.phase === "jack") return false;
    if (this.clubCooldown > 0) return true;
    ...
    if (!pick?.hit || !pick.pickedMesh || !pick.pickedPoint) return true;
    ...
    if (breakable) {
      if (breakable.requiredCharge && !charged) {
        ...
        if (breakable.health <= 0) this.breakBreakable(breakable);
      }
      return true;
    }
  ...
  public handleContextAction(): boolean {
    if (!this.active || this.traversalMotion) return false;
    ...
    if (!obstacle || Vector3.Distance(obstacle.mesh.position, playerPosition) > 3.2) return false;
    this.beginTraversal(obstacle);
    return true;
  }

  public throwObject(): boolean {
    if (!this.active || this.phase === "arrival" || this.phase === "jack") return false;
  ...
  public closeRearDoor(): boolean {
    ...
    if (!door || Vector3.Distance(door.mesh.position, position) > 3.8) return false;
    door.open = false;
  ...
  public canUseTorch(): boolean {
    ...
    if (this.torchDisabledByChapter) return false;
    if (this.playerInGasRoom() && this.gasConcentration > 0.28) return false;
    return this.replacementTorchOwned;
  }

  public setNoiseLevel(value: number): void {
    this.noise.restoreDanger(value);
  }

  public forceJesseSearch(): void {
    this.debugForceJesse = true;
    this.jesse.beginExploration();
    this.jesse.forceSearch(this.player.collider.position.add(this.player.forward().scale(7)));
  }

  public toggleNoiseVisualization(): boolean {
    return this.noise.toggleVisualization();
  }

  public toggleJesseVisualization(): boolean {
    return this.jesse.toggleVisualization();
  }

  public startJesseIntroduction(): void {
    this.enableForDebug();
  ...
  public startChaseSegment(index: number): void {
    this.enableForDebug();
  ...
  public grantGeneratorComponents(): void {
  ...
  public resetMelodyPuzzle(): void {
    this.melodyInput.length = 0;
    this.melodySolved = false;
    if (!this.generatorStates[3]) this.ui.toast("Sequência musical reiniciada.");
  }

  public spawnMiniboss(): void {
    this.enableForDebug();
  ...
  private createConnectedArchitecture(): void {
  ...
  private createMusicCorridor(): void {
  ...
  private createJackChamber(): void {
  ...
  private createChaseRoute(): void {
    this.chaseRoute.push(
      this.checkpoints.chase1.clone(),
      this.checkpoints.chase2.clone(),
      this.checkpoints.chase3.clone(),
      this.checkpoints.chase4.clone(),
      this.checkpoints.serviceElevator.clone()
    );
  ...
  private createServiceElevator(): void {
  ...
  private createMaintenanceNetwork(): void {
  ...
  private createGenerator2(): void {
    ...
    for (let index = 0; index < 6; index += 1) {
      ...
      const slot: ComponentSlot = { mesh: socket, index, installed: "" };
  ...
  private createGenerator5AndMaya(): void {
    ...
    this.interaction.register(this.generator5PlayerLever, {
      ...
      onInteract: () => {
        ...
        if (this.mayaEncounterState === "not-started") this.startMayaAssistance();
        else if (this.mayaEncounterState === "assisting" && this.mayaSequence >= 4.5) this.activateGenerator5();
      }
    });
  ...
  private createMimicObservationMirror(position: Vector3, normal: Vector3): void {
    const texture = new MirrorTexture(
      "mimic-observation-mirror-render",
      this.settings.performancePreset === "performance" ? 256 : 512,
      this.scene,
      true
    );
  ...
  private createHidingSpaces(): void {
    const positions = [
  ...
  private createThrownCanPool(): void {
    for (let index = 0; index < 5; index += 1) {
  ...
  private createJesseTraversalNodes(): void {
    const positions: Array<[string, number, number, string, boolean]> = [
  ...
  private updateJackSequence(deltaSeconds: number): void {
    if (this.jackSequence < 0) return;
  ...
  private updateChaseEnvironment(deltaSeconds: number): void {
    if (this.phase !== "chase") return;
    const playerPosition = this.player.collider.position;
    const shelf = this.scene.getMeshByName("chase-falling-shelf") as Mesh | null;
    ...
    if (this.settings.simplifiedChase && !this.traversalMotion) {
      ...
      if (obstacle) this.beginTraversal(obstacle);
    }
    if (this.chaseElevatorClosing) {
      this.chaseElevatorTimer += deltaSeconds;
      const doors = [this.scene.getMeshByName("service-elevator-door-left"), this.scene.getMeshByName("service-elevator-door-right")].filter(Boolean) as Mesh[];
      doors.forEach((door) => {
      ...
      if (this.chaseElevatorTimer > 3.3) this.completeChase();
    }
  }

  private updateTraversal(deltaSeconds: number): void {
    if (!this.traversalMotion) return;
    ...
    if (motion.type === "vault") position.y += Math.sin(t * Math.PI) * 1.35;
    else position.y = 0.12 - Math.sin(t * Math.PI) * 0.05;
  ...
  private updateThrownCans(deltaSeconds: number): void {
  ...
  private updateGasRoom(deltaSeconds: number): void {
    if (!this.active || this.generatorStates[2]) return;
  ...
  private updateMiniboss(deltaSeconds: number): void {
    if (!this.miniboss || this.miniboss.state === "dead" || this.miniboss.state === "dormant" || !this.miniboss.root.isEnabled()) return;
  ...
  private updateMemorySequence(deltaSeconds: number): void {
    if (this.memorySequence < 0) return;
    ...
    if (t > 4.4 && t - deltaSeconds <= 4.4) this.audio.stopJesseMelody();
    if (t > 0.8 && t - deltaSeconds <= 0.8) this.ui.showSubtitle("Voz fragmentada", "Maya... Daniel... Jesse... nós lembramos por você.", 4300);
  ...
  private checkAreaTriggers(): void {
    ...
    if (this.phase === "generators") {
      ...
      if (nearestGenerator && nearestGenerator.distance < 9) this.lastSafeCheckpoint = `generator-${nearestGenerator.index}`;
    }
  }

  private tryGenerator1(): void {
    if (!this.accessCardObtained && !this.inventory.has("generatorAccessCard")) {
  ...
  private interactComponentSlot(slot: ComponentSlot): void {
    if (slot.installed) {
  ...
  private tryGenerator4(): void {
    ...
    if (!correct) {
      ...
      if (!this.settings.extendedPuzzleWindows) this.jesse.forceSearch(this.checkpoints.generator4);
      return;
    }
  ...
  private completeGenerator(index: number): void {
    if (this.generatorStates[index]) return;
  ...
  private startMayaAssistance(): void {
  ...
  private activateGenerator5(): void {
    if (this.generatorStates[4]) return;
  ...
  private completeGeneratorVisualOnly(index: number): void {
    const rig = this.generators[index];
    if (!rig) return;
    rig.running = true;
    rig.light.diffuse = new Color3(0.22, 0.9, 0.35);
  ...
  private completeChase(): void {
    if (this.chaseComplete) return;
  ...
  private updateDanielObjective(): void {
  ...
  private generatorObjectiveText(): string {
    return `ATIVE OS CINCO GERADORES. ${this.generatorStates.filter(Boolean).length}/5`;
  }

  private onNoiseThreshold(): void {
    if (!this.active || this.phase !== "generators" || this.hiddenSpot || this.debugForceJesse) return;
  ...
  private updateDangerLights(deltaSeconds: number, danger: number): void {
    this.lightReaction = Math.max(0, this.lightReaction - deltaSeconds);
    const reactive = this.lightReaction > 0 || danger > 72;
    this.lights.forEach((light, index) => {
      if (reactive) light.intensity = 0.2 + Math.abs(Math.sin(performance.now() * 0.012 + index)) * 0.65;
      else light.intensity += (0.55 - light.intensity) * Math.min(1, deltaSeconds * 3);
    });
  }

  private handleJesseCaught(reason: "chase" | "search"): void {
    this.ui.flashDamage(1);
  ...
  private beginTraversal(obstacle: TraversalObstacle): void {
    ...
    if (obstacle.type === "slide") obstacle.mesh.position.y += 0.7;
    this.setChaseCheckpoint(obstacle.checkpoint);
  }

  private setChaseCheckpoint(index: number): void {
    const safe = Math.max(this.chaseCheckpoint, Math.min(4, index));
    if (safe === this.chaseCheckpoint) return;
    this.chaseCheckpoint = safe;
    this.callbacks.onCheckpoint(`jesse-chase-${safe}`);
  }

  private toggleHidingSpot(spot: HidingSpot): void {
    if (this.hiddenSpot && this.hiddenSpot !== spot) return;
    if (!spot.occupied) {
      spot.occupied = true;
  ...
  private emitNoise(category: NoiseCategory, intensity: number, material: NoiseMaterial, position: Vector3, duration?: number): void {
    this.noise.emit({ category, intensity, material, position, duration });
  }

  private isPathObstructed(from: Vector3, to: Vector3): boolean {
    const direction = to.subtract(from);
    const distance = direction.length();
    if (distance < 0.1) return false;
    direction.normalize();
    const pick = this.scene.pickWithRay(new Ray(from.add(new Vector3(0, 1.2, 0)), direction, distance), (mesh: AbstractMesh) =>
      mesh.checkCollisions && mesh.isEnabled() && mesh !== this.player.collider && !mesh.name.startsWith("jesse")
    );
    return Boolean(pick?.hit && pick.distance < distance - 0.4);
  }

  private playerInGasRoom(): boolean {
  ...
  private createRoomShell(
    ...
    const segments = [
      { side: "north" as const, pos: new Vector3(0, height / 2, depth / 2), size: { width, height, depth: 0.35 } },
      { side: "south" as const, pos: new Vector3(0, height / 2, -depth / 2), size: { width, height, depth: 0.35 } },
      { side: "east" as const, pos: new Vector3(width / 2, height / 2, 0), size: { width: 0.35, height, depth } },
      { side: "west" as const, pos: new Vector3(-width / 2, height / 2, 0), size: { width: 0.35, height, depth } }
    ];
  ...
  private createDoor(name: string, position: Vector3, slide: Vector3, lockable: boolean): DoorRig {
    ...
    const rig: DoorRig = { mesh, blocker, open: false, basePosition: position.clone(), slide: openOffset, lockable };
    this.doors.push(rig);
    return rig;
  }

  private createDoorFrame(name: string, position: Vector3, width: number, height: number, rotationY: number, openingSpan = width, wallHeight = 5.5): Mesh {
    ...
    if (upperHeight > 0.08) make("bulkhead-upper", { width: Math.max(openingSpan, width + jamb * 2), height: upperHeight, depth: 0.62 }, new Vector3(0, height / 2 + upperHeight / 2, 0));
  ...
  private toggleDoor(door: DoorRig): void {
    door.open = !door.open;
  ...
  private createGeneratorRig(index: number, position: Vector3): GeneratorRig {
    ...
    light.intensity = 0.5;
    light.range = 8;
    const rig: GeneratorRig = { root, console, light, running: false };
  ...
  private createCollectible(
    ...
    prompt: string,
    afterCollect: () => void,
    shape: "club" | "torch" | "recorder" | "card" | "seal"
  ): Mesh {
    let mesh: Mesh;
    if (shape === "club") {
  ...
  private createComponentMesh(id: string, position: Vector3, variant: number): Mesh {
    let mesh: Mesh;
    if (id === "rotor") mesh = MeshBuilder.CreateCylinder(`component-${id}`, { height: 0.9, diameter: 0.72, tessellation: 16 }, this.scene);
    else if (id === "insulator") mesh = MeshBuilder.CreateCylinder(`component-${id}`, { height: 0.75, diameterTop: 0.35, diameterBottom: 0.62, tessellation: 14 }, this.scene);
    else if (id === "brush") mesh = MeshBuilder.CreateBox(`component-${id}`, { width: 0.8, height: 0.45, depth: 0.55 }, this.scene);
    else if (id === "fuse") mesh = MeshBuilder.CreateCapsule(`component-${id}`, { height: 0.85, radius: 0.18, tessellation: 10 }, this.scene);
    else if (id === "regulator") mesh = MeshBuilder.CreateSphere(`component-${id}`, { diameter: 0.72, segments: 12 }, this.scene);
    else mesh = MeshBuilder.CreateTorus(`component-${id}`, { diameter: 0.82, thickness: 0.18, tessellation: 16 }, this.scene);
    mesh.parent = this.root;
    mesh.position.copyFrom(position);
  ...
  private createMiniboss(position: Vector3): MinibossRig {
  ...
  private isMinibossMesh(mesh: AbstractMesh): boolean {
    let current: AbstractMesh | null = mesh;
    while (current) {
      if (current.metadata?.gasMiniboss) return true;
      current = current.parent && "uniqueId" in current.parent ? current.parent as AbstractMesh : null;
    }
    return false;
  }

  private damageMiniboss(amount: number, charged: boolean): void {
    if (this.miniboss.state === "dead") return;
  ...
  private findBreakable(mesh: AbstractMesh): BreakableRig | null {
    let current: AbstractMesh | null = mesh;
    while (current) {
  ...
  private breakBreakable(rig: BreakableRig): void {
  ...
  private createProceduralSign(name: string, text: string, position: Vector3, rotationY: number, width: number, height: number, warning = false): Mesh {
  ...
  private applyRestoredState(): void {
    ...
    for (let index = 0; index < this.cableRoute.length; index += 1) {
      ...
      if (cable) cable.rotation.z = this.cableRoute[index]! * Math.PI / 3;
      const breaker = this.scene.getMeshByName(`generator1-breaker-${index}`);
      if (breaker) breaker.rotation.x = this.breakerStates[index] ? -0.45 : 0.45;
      const valve = this.scene.getMeshByName(`gas-vent-wheel-${index}`);
      if (valve) valve.rotation.z = this.gasVents[index] ? Math.PI / 2 : 0;
    }
    this.componentSlots.forEach((slot) => {
      if (slot.installed) slot.mesh.material = this.materials.get("plastic", 20 + this.componentOrder.indexOf(slot.installed));
    });
    ...
    if (cardReader && this.accessCardObtained) cardReader.material = this.materials.emissive("generator1-reader-restored", new Color3(0.18, 0.8, 0.32), 0.8);
    const gasSeal = this.scene.getMeshByName("gas-seal-item");
    if (gasSeal) gasSeal.setEnabled(!this.inventory.has("gasSeal") && !this.gasGeneratorRepaired);
    ...
    if (this.clubOwned) this.scene.getMeshByName("daniel-metal-club")?.setEnabled(false);
    if (this.replacementTorchOwned) this.scene.getMeshByName("daniel-replacement-torch")?.setEnabled(false);
    if (this.danielRecording) this.scene.getMeshByName("daniel-recorder")?.setEnabled(false);
    if (this.accessCardObtained) this.accessCardMesh.setEnabled(false);
