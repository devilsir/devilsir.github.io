...
export class MimicEntity {
    ...
    currentPreset;
    targetPreset;
    reflectionDelay = 0.38;
    reflectionOverrideTimer = 0;
    reflectedIdentity = "maya";
    ...
    setIdentity(identity, immediate = false) {
    ...
    setReflectionAnomaly(identity, duration = 2.4) {
        this.reflectedIdentity = identity;
    ...
    updateReveal(deltaSeconds) {
        ...
        if (p > 0.34 && p < 0.43 && this.identity !== "daniel")
            this.setIdentity("daniel");
        if (p > 0.5 && p < 0.59 && this.identity !== "child")
            this.setIdentity("child");
        if (p > 0.66 && p < 0.75 && this.identity !== "employee")
            this.setIdentity("employee");
        if (p > 0.8 && this.identity !== "composite")
            this.setIdentity("composite");
        if (p >= 1) {
    ...
    redrawFace(identity, corruption) {
    ...
    createLimbs() {
        ...
        const makeSegments = (root, name, arm) => {
            const segments = [];
            let parent = root;
