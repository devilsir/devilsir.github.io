...
export class BodyBoss {
    ...
    charging = false;
    defeated = false;
    extendedReactionWindows = false;
    constructor(scene, materials, player, fire, audio, ui, callbacks) {
        this.scene = scene;
        ...
        fire.register(this.seam, {
            ...
            onBurnTick: (damage) => {
                if (this.phase >= 2 && this.stunned)
                    this.applyDamage(damage * 0.19, true);
            },
            onDestroyed: () => {
    ...
    setExtendedReactionWindows(enabled) {
        this.extendedReactionWindows = enabled;
    }
    markDefeatedForRestore() {
        this.active = false;
        this.defeated = true;
        this.health = 0;
        this.phase = 3;
    ...
    activateSpotlight() {
        if (!this.active || this.phase !== 2 || this.stunned)
            return;
    ...
    performTacticalAction(action) {
        ...
        if (action === "attack") {
            if (this.phase === 2 && !this.stunned)
                this.callbacks.onHint("O tecido frontal é grosso demais.");
            else
                this.applyDamage(this.phase === 1 ? 8 : 5, false);
        }
    ...
    update(deltaSeconds) {
        ...
        if (this.tacticalTimer <= 0 && !this.tacticalOpen)
            this.openTacticalWindow();
        ...
        if (this.phase === 3 && this.debris.length < 8 && Math.random() < deltaSeconds * 0.34)
            this.spawnCeilingDebris();
    }
    chooseAttack(distance, direction) {
    ...
    updateDebris(deltaSeconds) {
        for (let index = this.debris.length - 1; index >= 0; index -= 1) {
            ...
            if (piece.mesh.position.y <= 0.35) {
                if (Vector3.Distance(piece.mesh.position, this.player.collider.position) < 1.65)
                    this.hitPlayer(14);
                this.audio.impact(0.8);
                this.callbacks.onArenaDamage(1);
                piece.mesh.dispose();
    ...
    applyDamage(amount, environmental) {
        if (!this.active || this.defeated)
            return;
        if (this.phase === 2 && !environmental && !this.stunned)
            return;
