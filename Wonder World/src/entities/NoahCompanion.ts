...
export type NoahCommand = "follow" | "wait" | "move" | "hold" | "crawl" | "distract";

export interface NoahSaveState {
  active: boolean;
  command: NoahCommand;
  checkpoint: string;
...
export interface NoahCallbacks {
  onReachedTarget?: (tag: string) => void;
  onStuckRecovery?: () => void;
}

export class NoahCompanion {
  public active = false;
  public command: NoahCommand = "wait";
  public trust = 0;
  public injured = true;
  public carryingIdentityCard = false;
  public checkpoint = "noah-introduction";

  private readonly scene: Scene;
  private readonly player: PlayerController;
  private readonly audio: AudioManager;
  private readonly callbacks: NoahCallbacks;
  ...
  private hiddenRecoveryAnchor: Vector3 | null = null;
  private commandClock = 0;
  ...
  public moveTo(position: Vector3, tag = "marked-position"): void {
    this.command = "move";
  ...
  public holdAt(position: Vector3, tag = "held-mechanism"): void {
    this.command = "hold";
    this.target = position.clone();
    this.targetTag = tag;
  }

  public crawlTo(position: Vector3, tag = "crawl-control"): void {
    this.command = "crawl";
    this.target = position.clone();
    this.targetTag = tag;
  }

  public distract(position: Vector3): void {
  ...
  public setHiddenRecoveryAnchor(position: Vector3 | null): void {
  ...
  public update(deltaSeconds: number): void {
    ...
    let destination: Vector3 | null = this.target?.clone() ?? null;
    ...
    if (distance < (this.command === "follow" ? 1.35 : 0.72)) {
      ...
      if (this.command !== "follow") {
        ...
        if (this.command === "hold") this.poseHolding();
        else if (this.command === "crawl") this.poseCrouched();
        else this.command = "wait";
        this.target = null;
        this.targetTag = "";
    ...
    if (distance >= this.lastDistance - 0.015) this.stuckClock += deltaSeconds;
    else this.stuckClock = Math.max(0, this.stuckClock - deltaSeconds * 2);
    this.lastDistance = distance;
    if (this.stuckClock > 2.8 && this.recoverCooldown <= 0) this.recoverFromStuck(destination);

    if (this.command === "distract" && this.commandClock <= 0) this.setFollow();
  }

  public serialize(): NoahSaveState {
    return {
      active: this.active,
  ...
  public restore(state: NoahSaveState): void {
    ...
    if (state.command === "follow") this.setFollow();
  }
  ...
  private createLimb(parent: TransformNode, side: number, length: number, upperMaterial: PBRMaterial, skinMaterial: PBRMaterial, name: string): void {
  ...
  private createLeg(parent: TransformNode, side: number, material: PBRMaterial, name: string): void {
  ...
  private poseWalking(speed: number): void {
  ...
  private recoverFromStuck(destination: Vector3): void {
    this.stuckClock = 0;
