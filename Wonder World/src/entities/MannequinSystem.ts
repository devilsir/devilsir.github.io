...
export type MannequinState = "dormant" | "stalking" | "running" | "crawler" | "crushed" | "inactive";

export interface MannequinSpawn {
  position: Vector3;
  rotationY?: number;
  group?: number;
  pose?: number;
...
export interface MannequinDebugSnapshot {
  enabled: number;
  observed: number;
  activeActors: number;
  running: number;
...
export interface MirrorObservationResolver {
  (mannequinPosition: Vector3): boolean;
}
...
export class MannequinSystem {
  ...
  private readonly records: MannequinRecord[] = [];
  private readonly actors: ActorPoolEntry[] = [];
  private readonly lowDetailSource: Mesh;
  private readonly lowDetailMatrices: Float32Array;
  private readonly occlusionLines: Mesh[] = [];
  private mirrorResolver: MirrorObservationResolver = () => false;
  private movementEnabled = true;
  private chapterEnabled = false;
  private lightingLevel = 1;
  private rulesCollapsed = 0;
  private updateAccumulator = 0;
  private matrixAccumulator = 0;
  private groupCursor = 0;
  private lurePosition: Vector3 | null = null;
  private lureExpiresAt = 0;
  private observationVisualization = false;
  private occlusionVisualization = false;
  private aiCostMs = 0;
  private lastRearCue = 0;
  private countLimit: number;
  private readonly highDetailLimit: number;
  ...
  public constructor(
    ...
    for (let index = 0; index < this.highDetailLimit; index += 1) this.actors.push(this.createActor(index));
    this.refreshThinInstances(true);
  }

  public setChapterEnabled(enabled: boolean): void {
    this.chapterEnabled = enabled;
    this.lowDetailSource.setEnabled(enabled);
    if (!enabled) this.actors.forEach((actor) => actor.root.setEnabled(false));
  }
  ...
  public setLightingLevel(level: number): void {
    this.lightingLevel = Math.max(0, Math.min(1, level));
  }

  public setRuleCollapse(progress: number): void {
    this.rulesCollapsed = Math.max(0, Math.min(1, progress));
  }

  public setMirrorResolver(resolver: MirrorObservationResolver): void {
    this.mirrorResolver = resolver;
  }

  public setCount(count: number): void {
    this.countLimit = Math.max(8, Math.min(this.records.length, Math.round(count)));
    for (const record of this.records) record.enabled = record.id < this.countLimit && record.state !== "inactive";
    this.refreshThinInstances(true);
  }
  ...
  public toggleOcclusionVisualization(): boolean {
    ...
    if (!this.occlusionVisualization) this.clearOcclusionLines();
    return this.occlusionVisualization;
  }

  public emitLure(position: Vector3, strength = 1, duration = 8): void {
    ...
    for (const record of this.records) {
      if (!record.enabled || record.state === "crushed" || record.state === "inactive") continue;
      const distance = Vector3.Distance(record.position, position);
      if (distance < 18 + strength * 14 && record.state === "dormant") record.state = "stalking";
    }
  }

  public crushInRadius(center: Vector3, radius: number): number {
    let crushed = 0;
    for (const record of this.records) {
      if (!record.enabled || record.state === "crushed" || record.state === "inactive") continue;
      if (Vector3.Distance(record.position, center) > radius) continue;
    ...
    if (crushed > 0) this.refreshThinInstances(true);
    return crushed;
  }

  public deactivateNearest(position: Vector3, radius: number): MannequinRecord | null {
    let selected: MannequinRecord | null = null;
    let distance = radius;
    for (const record of this.records) {
      if (!record.enabled || record.state === "inactive" || record.state === "crushed") continue;
  ...
  public attackNearest(origin: Vector3, forward: Vector3, range: number, damage: number, charged: boolean): boolean {
    let selected: MannequinRecord | null = null;
    let bestScore = Number.POSITIVE_INFINITY;
    const horizontalForward = forward.clone();
    horizontalForward.y = 0;
    if (horizontalForward.lengthSquared() < 0.001) return false;
    ...
    push.y = 0;
    if (push.lengthSquared() > 0.001) selected.position.addInPlace(push.normalize().scale(charged ? 1.5 : 0.7));
  ...
  public resetForPlayerRespawn(): void {
    ...
    for (const record of this.records) {
      if (record.state === "crushed" || record.state === "inactive" || !record.enabled) continue;
  ...
  public getReflectableMeshes(): AbstractMesh[] {
    const meshes: AbstractMesh[] = [this.lowDetailSource];
    for (const actor of this.actors) meshes.push(...actor.skinMeshes, ...actor.joints);
    return meshes;
  }

  public getNearestRearDistance(): number {
    ...
    for (const record of this.records) {
      ...
      const distance = offset.length();
      if (distance > 10 || distance < 0.001) continue;
      if (Vector3.Dot(offset.scale(1 / distance), forward) < -0.18) nearest = Math.min(nearest, distance);
    }
    return nearest;
  }

  public update(deltaSeconds: number): void {
    if (!this.chapterEnabled) return;
    ...
    if (this.lurePosition && now >= this.lureExpiresAt) this.lurePosition = null;

    const updateGroup = this.groupCursor % 4;
    this.groupCursor += 1;
    const raySamples: Array<{ from: Vector3; to: Vector3; blocked: boolean }> = [];
    for (const record of this.records) {
      if (!record.enabled || record.id >= this.countLimit || record.state === "inactive") continue;
      const distance = Vector3.Distance(record.position, this.player.collider.position);
      if (distance > 55 && record.group % 4 !== updateGroup) continue;
      const observedResult = this.evaluateObservation(record, now);
      record.observed = observedResult.observed;
      if (observedResult.ray) raySamples.push(observedResult.ray);
    ...
    if (this.occlusionVisualization) this.drawOcclusionLines(raySamples.slice(0, 12));
    this.aiCostMs = this.aiCostMs * 0.72 + (performance.now() - start) * 0.28;
  }

  public inspect(): MannequinDebugSnapshot {
    let enabled = 0;
    let observed = 0;
    let running = 0;
    let crawling = 0;
    for (const record of this.records) {
      if (!record.enabled || record.id >= this.countLimit) continue;
      enabled += 1;
      if (record.observed) observed += 1;
      if (record.state === "running") running += 1;
      if (record.state === "crawler") crawling += 1;
    }
    return {
      enabled,
      observed,
  ...
  private evaluateObservation(
    record: MannequinRecord,
    now: number
  ): { observed: boolean; ray?: { from: Vector3; to: Vector3; blocked: boolean } } {
    if (this.lightingLevel < 0.14) return { observed: false };
    ...
    const distance = offset.length();
    if (distance < 0.01 || distance > 38) return { observed: now < record.observedUntil };
    ...
    const projected = Vector3.Project(
      ...
      this.player.camera.viewport.toGlobal(this.scene.getEngine().getRenderWidth(), this.scene.getEngine().getRenderHeight())
    );
    ...
    let blocked = false;
    if (directCandidate) {
      const pick = this.scene.pickWithRay(new Ray(cameraPosition, direction, distance), (mesh: AbstractMesh) => {
        if (!mesh.isPickable || !mesh.isVisible || !mesh.isEnabled()) return false;
        if (mesh === this.player.collider || mesh.metadata?.mannequinVisual) return false;
        return true;
      });
    ...
    if (observed) record.observedUntil = now + 0.22;
    return {
  ...
  private updateRecord(record: MannequinRecord, deltaSeconds: number, now: number, distanceToPlayer: number): void {
    if (record.state === "crushed" || record.state === "inactive") return;
    ...
    if (distanceToPlayer > 30 && !this.lurePosition) record.state = "dormant";
    else if (record.state === "dormant" && (distanceToPlayer < 19 || this.lurePosition)) record.state = "stalking";
    if (this.rulesCollapsed > 0.38 && distanceToPlayer < 28) record.state = record.health < 45 ? "crawler" : "running";
    ...
    desired.y = 0;
    if (desired.lengthSquared() < 0.02) return;
    desired.normalize();
  ...
  private navigationDirection(record: MannequinRecord, desired: Vector3): Vector3 {
    const origin = record.position.add(new Vector3(0, 0.8, 0));
    const pick = this.scene.pickWithRay(new Ray(origin, desired, 1.25), (mesh: AbstractMesh) => {
      if (!mesh.checkCollisions || mesh === this.player.collider || mesh.metadata?.mannequinVisual) return false;
      return mesh.isEnabled();
    });
    if (!pick?.hit || pick.distance > 0.95) return desired;
    const side = record.id % 2 === 0 ? 1 : -1;
    return new Vector3(desired.z * side, 0, -desired.x * side).normalize();
  }

  private assignActors(): void {
    ...
    for (const entry of candidates) {
      if (entry.record.activeSlot >= 0) continue;
  ...
  private animateActors(deltaSeconds: number): void {
    const now = performance.now() * 0.001;
    for (const actor of this.actors) {
      if (actor.recordId < 0) continue;
  ...
  private refreshThinInstances(force: boolean): void {
    if (!force && !this.chapterEnabled) return;
    for (const record of this.records) {
  ...
  private createActor(index: number): ActorPoolEntry {
  ...
  private updateRearWarning(now: number): void {
    const rearDistance = this.getNearestRearDistance();
    if (rearDistance >= 5.5 || now - this.lastRearCue < 2.1) return;
    this.lastRearCue = now;
    this.audio.rearProximity(rearDistance);
    if (rearDistance < 3.2) this.ui.showSoundCaption("passos secos se aproximam atrás de você", 1500);
  }

  private drawOcclusionLines(samples: Array<{ from: Vector3; to: Vector3; blocked: boolean }>): void {
  ...
  private clearOcclusionLines(): void {
    while (this.occlusionLines.length > 0) this.occlusionLines.pop()?.dispose();
  }
}
