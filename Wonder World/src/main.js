...
function showBootstrapError(error) {
  ...
  `;
    ...
    document.querySelector("#boot-safe-mode")?.addEventListener("click", () => location.assign(safeModeUrl()));
    document.querySelector("#boot-copy-error")?.addEventListener("click", () => {
        void navigator.clipboard?.writeText(report);
    });
}
function showPreviousCrashRecovery() {
    if (!bootFallback)
        return;
  ...
  `;
    document.querySelector("#boot-safe-mode")?.addEventListener("click", () => location.assign(safeModeUrl()));
    document.querySelector("#boot-normal-retry")?.addEventListener("click", () => {
        localStorage.removeItem(BOOT_MARKER);
        const next = new URL(location.href);
