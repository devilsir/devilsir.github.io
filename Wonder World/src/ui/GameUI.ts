...
export type MenuAction = "new" | "continue" | "load" | "tips" | "settings" | "accessibility" | "credits";
...
export interface FinalCreditStats {
  title: string;
  ending: string;
  documents: number;
  deaths: number;
...
export interface DebugCallbacks {
  ...
  giveItems: () => void;
  toggleInfiniteFuel: () => void;
  toggleInvulnerability: () => void;
  resetPuzzle: () => void;
  startBoss: () => void;
  bossPhase: (phase: number) => void;
  toggleCollisions: () => void;
  toggleMannequinMovement: () => void;
  toggleObservationVisualization: () => void;
  toggleOcclusionVisualization: () => void;
  forceBlackout: () => void;
  grantEnergyCells: () => void;
  resetMirrorPuzzle: () => void;
  resetShelfPuzzle: () => void;
  startSphereArena: (arena: number) => void;
  setMannequinCount: (count: number) => void;
  startJesseIntroduction: () => void;
  startChaseSegment: (segment: number) => void;
  setNoiseLevel: (level: number) => void;
  forceJesseSearch: () => void;
  toggleNoiseVisualization: () => void;
  toggleJesseVisualization: () => void;
  grantGeneratorComponents: () => void;
  resetMelodyPuzzle: () => void;
  spawnMiniboss: () => void;
  triggerMayaReveal: () => void;
  previewMimicIdentity: (identity: "maya" | "daniel" | "employee" | "child" | "blank" | "composite") => void;
  resetFragileFloor: () => void;
  revealFloorSolution: () => void;
  triggerBadEnding: () => void;
  grantChapter4Equipment: () => void;
  forceMimicEvidence: (evidence: "wrong-memory" | "mirrored-hand" | "shared-breath" | "missing-reflection" | "mixed-clothing" | "wrong-injury" | "delayed-expression" | "impossible-position") => void;
  startMimicBossPhase: (phase: number) => void;
  setPostDefeatAttackCount: (count: number) => void;
  inspectEndingFlags: () => string;
  setNoahCommand: (command: "follow" | "wait" | "move" | "distract") => void;
  triggerTrueEnding: () => void;
  inspectChapter5: () => string;
  refreshState: () => string;
}
...
export class GameUI {
  ...
  private menuActionHandler: ((action: MenuAction) => void) | null = null;
  private pauseHandler: ((action: "resume" | "tips" | "settings" | "menu") => void) | null = null;
  private tacticalHandler: ((action: string) => void) | null = null;
  private toastTimer = 0;
  private subtitleTimer = 0;
  private captionTimer = 0;
  private documentCloseHandler: (() => void) | null = null;

  public constructor(root: HTMLDivElement, settings: GameSettings) {
    this.root = root;
  ...
  public ensureGameplayVisible(): void {
  ...
  public showFatalError(error: unknown): void {
    ...
    this.get<HTMLButtonElement>("fatal-error-reload").onclick = () => location.reload();
    this.get<HTMLButtonElement>("fatal-error-copy").onclick = () => {
      void navigator.clipboard?.writeText(details.textContent ?? message);
      this.toast("Erro copiado.");
    };
  }

  public bindMenu(handler: (action: MenuAction) => void): void {
    this.menuActionHandler = handler;
    this.root.querySelectorAll<HTMLButtonElement>("[data-menu-action]").forEach((button) => {
      button.addEventListener("click", () => {
        const action = button.dataset.menuAction as MenuAction | undefined;
        if (action) this.menuActionHandler?.(action);
      });
    });
  }

  public setContinueEnabled(enabled: boolean): void {
    const continueButton = this.root.querySelector<HTMLButtonElement>("[data-menu-action='continue']");
    const loadButton = this.root.querySelector<HTMLButtonElement>("[data-menu-action='load']");
  ...
  public showPause(handler: (action: "resume" | "tips" | "settings" | "menu") => void): void {
  ...
  public showInventory(items: InventoryItem[], onClose: () => void): void {
    ...
    this.get<HTMLButtonElement>("inventory-close").onclick = onClose;
  }
  ...
  public showSettings(settings: GameSettings, callbacks: SettingsCallbacks, accessibilityOnly = false): void {
    this.currentSettings = settings;
    const panel = this.get<HTMLDivElement>("settings-screen");
    panel.classList.remove("hidden");
    panel.querySelector<HTMLElement>("[data-settings-title]")!.textContent = accessibilityOnly
      ? "ACESSIBILIDADE"
      : "CONFIGURAÇÕES";

    const fields = panel.querySelectorAll<HTMLInputElement | HTMLSelectElement>("[data-setting]");
    fields.forEach((field) => {
      const key = field.dataset.setting as keyof GameSettings | undefined;
      if (!key) return;
      ...
      const row = field.closest<HTMLElement>("[data-setting-row]");
      if (row) {
    ...
    const saveButton = panel.querySelector<HTMLButtonElement>("[data-settings-save]")!;
    const resetButton = panel.querySelector<HTMLButtonElement>("[data-settings-reset]")!;
    const closeButton = panel.querySelector<HTMLButtonElement>("[data-settings-close]")!;
    ...
    saveButton.onclick = () => {
      ...
      fields.forEach((field) => {
        ...
        if (field instanceof HTMLInputElement && field.type === "range") value = Number(field.value);
  ...
  public showFinalCredits(stats: FinalCreditStats, onComplete: () => void): void {
    ...
    const button = overlay.querySelector<HTMLButtonElement>(".credits-skip")!;
    const finish = (): void => {
    ...
    window.setTimeout(() => { if (overlay.isConnected) finish(); }, 33000);
  }

  public showPostCredits(onComplete: () => void): void {
    ...
    `;
    this.root.appendChild(overlay);
    const indicator = overlay.querySelector<HTMLElement>(".elevator-indicator")!;
    ...
    const button = overlay.querySelector<HTMLButtonElement>(".post-credits-close")!;
    window.setTimeout(() => { button.disabled = false; }, 11600);
    const finish = (): void => { overlay.remove(); onComplete(); };
    button.onclick = finish;
    window.setTimeout(() => { if (overlay.isConnected) finish(); }, 16000);
  }

  public setEndingGallery(endings: string[]): void {
  ...
  public showCredits(): void {
    ...
    this.get<HTMLButtonElement>("credits-close").onclick = () => this.hideElement("credits-screen");
  }

  public showTips(objective: string, hints: string[], onClose?: () => void): void {
    ...
    this.get<HTMLButtonElement>("tips-close").onclick = () => {
  ...
  public updateStatus(health: number, fuel: number, inventory: InventoryItem[], lives = 3, armor = 0): void {
  ...
  public setNoiseDanger(percent: number, visible: boolean): void {
  ...
  public setStamina(percent: number): void {
    this.get("stamina-fill").style.width = `${Math.max(0, Math.min(100, percent))}%`;
  }

  public showSubtitle(speaker: string, text: string, duration = 4200): void {
    if (!this.currentSettings.subtitles) return;
  ...
  public showSoundCaption(text: string, duration = 2600): void {
    if (!this.currentSettings.soundCaptions) return;
  ...
  public toast(text: string, duration = 2600): void {
  ...
  public showEnding(title: string, body: string, onRetry: () => void, onMenu: () => void): void {
    this.get("ending-title").textContent = title;
    this.get("ending-body").textContent = body;
    this.showElement("ending-screen");
    this.get<HTMLButtonElement>("ending-retry").onclick = () => {
      this.hideElement("ending-screen");
      onRetry();
    };
    this.get<HTMLButtonElement>("ending-menu").onclick = () => {
      this.hideElement("ending-screen");
      onMenu();
    };
  }

  public setCorruption(amount: number, duration = 1100): void {
  ...
  public showDocument(title: string, body: string, onClose?: () => void): void {
    ...
    this.get<HTMLButtonElement>("document-close").onclick = () => this.closeDocument();
  }
   ...
   */
  public dismissGameplayText(): boolean {
  ...
  public async playOpeningFilm(onSkip: () => void): Promise<void> {
    const screen = this.get("cinematic");
    const canvas = this.get<HTMLCanvasElement>("film-canvas");
    ...
    window.setTimeout(() => {
      if (!skipped) window.speechSynthesis.speak(narration);
    }, 650);
    this.get<HTMLButtonElement>("skip-film").onclick = () => {
      skipped = true;
      window.speechSynthesis.cancel();
  ...
  public showChapterCard(title: string, subtitle = "", duration = 3600): Promise<void> {
  ...
  public showBossUI(name: string): void {
  ...
  public setBossHealth(percent: number): void {
    this.get("boss-health-fill").style.width = `${Math.max(0, Math.min(100, percent))}%`;
  }

  public showTacticalActions(actions: { id: string; label: string; enabled?: boolean }[], handler: (action: string) => void): void {
  ...
  public hideTacticalActions(): void {
    this.get("tactical-actions").classList.add("hidden");
  }

  public flashDamage(strength = 1): void {
  ...
  public flashLightning(): void {
    if (this.currentSettings.reducedFlashing) return;
  ...
  public showDebug(callbacks: DebugCallbacks): void {
    const panel = this.get("debug-panel");
    panel.classList.toggle("hidden");
    if (panel.classList.contains("hidden")) return;
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-teleport]").forEach((button) => {
      button.onclick = () => callbacks.teleport(button.dataset.debugTeleport ?? "prologue");
    });
    panel.querySelector<HTMLButtonElement>("[data-debug-give]")!.onclick = callbacks.giveItems;
    panel.querySelector<HTMLButtonElement>("[data-debug-fuel]")!.onclick = callbacks.toggleInfiniteFuel;
    panel.querySelector<HTMLButtonElement>("[data-debug-god]")!.onclick = callbacks.toggleInvulnerability;
    panel.querySelector<HTMLButtonElement>("[data-debug-reset]")!.onclick = callbacks.resetPuzzle;
    panel.querySelector<HTMLButtonElement>("[data-debug-boss]")!.onclick = callbacks.startBoss;
    panel.querySelector<HTMLButtonElement>("[data-debug-collisions]")!.onclick = callbacks.toggleCollisions;
    panel.querySelector<HTMLButtonElement>("[data-debug-mannequins]")!.onclick = callbacks.toggleMannequinMovement;
    panel.querySelector<HTMLButtonElement>("[data-debug-observation]")!.onclick = callbacks.toggleObservationVisualization;
    panel.querySelector<HTMLButtonElement>("[data-debug-rays]")!.onclick = callbacks.toggleOcclusionVisualization;
    panel.querySelector<HTMLButtonElement>("[data-debug-blackout]")!.onclick = callbacks.forceBlackout;
    panel.querySelector<HTMLButtonElement>("[data-debug-cells]")!.onclick = callbacks.grantEnergyCells;
    panel.querySelector<HTMLButtonElement>("[data-debug-mirror]")!.onclick = callbacks.resetMirrorPuzzle;
    panel.querySelector<HTMLButtonElement>("[data-debug-shelves]")!.onclick = callbacks.resetShelfPuzzle;
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-arena]").forEach((button) => {
      button.onclick = () => callbacks.startSphereArena(Number(button.dataset.debugArena));
    });
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-count]").forEach((button) => {
      button.onclick = () => callbacks.setMannequinCount(Number(button.dataset.debugCount));
    });
    panel.querySelector<HTMLButtonElement>("[data-debug-jesse-intro]")!.onclick = callbacks.startJesseIntroduction;
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-chase]").forEach((button) => {
      button.onclick = () => callbacks.startChaseSegment(Number(button.dataset.debugChase));
    });
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-noise]").forEach((button) => {
      button.onclick = () => callbacks.setNoiseLevel(Number(button.dataset.debugNoise));
    });
    panel.querySelector<HTMLButtonElement>("[data-debug-jesse-search]")!.onclick = callbacks.forceJesseSearch;
    panel.querySelector<HTMLButtonElement>("[data-debug-noise-view]")!.onclick = callbacks.toggleNoiseVisualization;
    panel.querySelector<HTMLButtonElement>("[data-debug-jesse-view]")!.onclick = callbacks.toggleJesseVisualization;
    panel.querySelector<HTMLButtonElement>("[data-debug-generator-components]")!.onclick = callbacks.grantGeneratorComponents;
    panel.querySelector<HTMLButtonElement>("[data-debug-melody-reset]")!.onclick = callbacks.resetMelodyPuzzle;
    panel.querySelector<HTMLButtonElement>("[data-debug-miniboss]")!.onclick = callbacks.spawnMiniboss;
    panel.querySelector<HTMLButtonElement>("[data-debug-maya]")!.onclick = callbacks.triggerMayaReveal;
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-mimic]").forEach((button) => {
      button.onclick = () => callbacks.previewMimicIdentity((button.dataset.debugMimic ?? "maya") as "maya" | "daniel" | "employee" | "child" | "blank" | "composite");
    });
    panel.querySelector<HTMLButtonElement>("[data-debug-floor-reset]")!.onclick = callbacks.resetFragileFloor;
    panel.querySelector<HTMLButtonElement>("[data-debug-floor-solution]")!.onclick = callbacks.revealFloorSolution;
    panel.querySelector<HTMLButtonElement>("[data-debug-bad-ending]")!.onclick = callbacks.triggerBadEnding;
    panel.querySelector<HTMLButtonElement>("[data-debug-ch4-equipment]")!.onclick = callbacks.grantChapter4Equipment;
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-mimic-evidence]").forEach((button) => {
      button.onclick = () => callbacks.forceMimicEvidence((button.dataset.debugMimicEvidence ?? "wrong-memory") as "wrong-memory" | "mirrored-hand" | "shared-breath" | "missing-reflection" | "mixed-clothing" | "wrong-injury" | "delayed-expression" | "impossible-position");
    });
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-mimic-boss-phase]").forEach((button) => {
      button.onclick = () => callbacks.startMimicBossPhase(Number(button.dataset.debugMimicBossPhase));
    });
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-post-attacks]").forEach((button) => {
      button.onclick = () => callbacks.setPostDefeatAttackCount(Number(button.dataset.debugPostAttacks));
    });
    panel.querySelector<HTMLButtonElement>("[data-debug-ending-flags]")!.onclick = () => this.toast(callbacks.inspectEndingFlags(), 5200);
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-noah]").forEach((button) => {
      button.onclick = () => callbacks.setNoahCommand((button.dataset.debugNoah ?? "follow") as "follow" | "wait" | "move" | "distract");
    });
    panel.querySelector<HTMLButtonElement>("[data-debug-true-ending]")!.onclick = callbacks.triggerTrueEnding;
    panel.querySelector<HTMLButtonElement>("[data-debug-ch5-inspect]")!.onclick = () => this.toast(callbacks.inspectChapter5(), 7000);
    panel.querySelectorAll<HTMLButtonElement>("[data-debug-phase]").forEach((button) => {
  ...
  private hideAllScreens(): void {
    this.root.querySelectorAll<HTMLElement>(".screen").forEach((element) => element.classList.add("hidden"));
  }
  ...
  private drawFilmFrame(context: CanvasRenderingContext2D, canvas: HTMLCanvasElement, t: number): void {
    ...
    if (t < 2.1) {
      ...
      context.fillStyle = "#111";
      for (let i = 0; i < 6; i += 1) this.drawPerson(context, 140 + i * 125, 420, 0.5, false);
  ...
  private drawPerson(context: CanvasRenderingContext2D, x: number, y: number, scale: number, child: boolean): void {
  ...
  private drawMascot(context: CanvasRenderingContext2D, x: number, y: number, scale: number, burned: boolean): void {
    context.save();
    context.translate(x, y);
    context.scale(scale, scale);
  ...
  private drawToy(context: CanvasRenderingContext2D, x: number, y: number, scale: number): void {
    context.save();
    context.translate(x, y);
    context.scale(scale, scale);
  ...
  public bindPauseButtons(): void {
    this.get<HTMLButtonElement>("pause-resume").onclick = () => this.pauseHandler?.("resume");
    this.get<HTMLButtonElement>("pause-tips").onclick = () => this.pauseHandler?.("tips");
    this.get<HTMLButtonElement>("pause-settings").onclick = () => this.pauseHandler?.("settings");
    this.get<HTMLButtonElement>("pause-main-menu").onclick = () => this.pauseHandler?.("menu");
  }

  private rangeRow(label: string, key: keyof GameSettings, min: number, max: number, step: number, accessibility = false): string {
  ...
  private checkboxRow(label: string, key: keyof GameSettings, accessibility = false): string {
    const id = `setting-${key}`;
  ...
  private selectRow(label: string, key: keyof GameSettings, options: [string, string][], accessibility = false): string {
    const id = `setting-${key}`;
