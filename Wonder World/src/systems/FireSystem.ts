...
export interface BurnableDefinition {
  health: number;
  spreadRadius?: number;
  onIgnite?: () => void;
  onBurnTick?: (damage: number) => void;
  onDestroyed?: () => void;
}

interface BurnableRuntime extends BurnableDefinition {
  mesh: AbstractMesh;
  burning: boolean;
...
interface FireInstance {
  ...
  age: number;
  duration: number;
  source: BurnableRuntime | null;
}

export class FireSystem {
  public fuel = 0;
  public torchLit = false;
  public infiniteFuel = false;
  private torchAvailable = false;
  private readonly scene: Scene;
  private readonly camera: Camera;
  private readonly audio: AudioManager;
  private readonly materials: ProceduralMaterials;
  private readonly torchRoot: TransformNode;
  private readonly torchLight: PointLight;
  private readonly torchFlames: Mesh[] = [];
  private readonly burnables = new Map<number, BurnableRuntime>();
  private readonly fires: FireInstance[] = [];
  private readonly scorchMarks: Mesh[] = [];
  private readonly maxFires = 18;
  private readonly maxScorches = 28;
  private updateAccumulator = 0;

  public constructor(scene: Scene, camera: Camera, audio: AudioManager, materials: ProceduralMaterials) {
    this.scene = scene;
    this.camera = camera;
    this.audio = audio;
  ...
  public register(mesh: AbstractMesh, definition: BurnableDefinition): void {
  ...
  public setTorchAvailable(available: boolean): void {
    this.torchAvailable = available;
    if (!available) this.torchLit = false;
    this.torchRoot.setEnabled(available);
  }

  public toggleTorch(): boolean {
    ...
    if (this.fuel <= 0 && !this.infiniteFuel) return false;
    this.torchLit = true;
    this.audio.fireBurst();
    return true;
  }

  public useTorch(): AbstractMesh | null {
    if (!this.torchLit) return null;
  ...
  public igniteMesh(mesh: AbstractMesh): boolean {
    const runtime = this.findBurnable(mesh);
    if (!runtime || runtime.burning || runtime.currentHealth <= 0) return false;
    this.ignite(runtime);
    return true;
  }

  public igniteAt(position: Vector3, duration = 12, withLight = true): void {
    if (this.fires.length >= this.maxFires) {
      const oldest = this.fires.shift();
      if (oldest) this.disposeFire(oldest);
    }
    ...
    if (position.y < 1.2) this.createScorchMark(position);
  }

  public isFireNear(position: Vector3, radius = 2): boolean {
    return this.isTorchThreatNear(position, radius)
      || this.fires.some((fire) => Vector3.Distance(fire.root.position, position) <= radius);
  }

  public isTorchThreatNear(position: Vector3, radius = 5.5): boolean {
    if (!this.torchAvailable || !this.torchLit) return false;
    return Vector3.Distance(this.camera.globalPosition, position) <= radius;
  }

  public getTorchThreatPosition(): Vector3 | null {
    return this.torchAvailable && this.torchLit ? this.camera.globalPosition.clone() : null;
  }

  public nearestFireDistance(position: Vector3): number {
    let nearest = Infinity;
    for (const fire of this.fires) nearest = Math.min(nearest, Vector3.Distance(fire.root.position, position));
    return nearest;
  }

  public reset(): void {
    for (const fire of this.fires) this.disposeFire(fire);
    this.fires.length = 0;
  ...
  private ignite(runtime: BurnableRuntime): void {
    if (runtime.burning) return;
    ...
    if (fire) fire.source = runtime;
  }

  private update(deltaSeconds: number): void {
    if (!this.torchAvailable) this.torchLit = false;
    ...
    if (this.torchLit && !this.infiniteFuel) {
      ...
      if (this.fuel <= 0) this.torchLit = false;
    }
    ...
    for (let index = this.fires.length - 1; index >= 0; index -= 1) {
      ...
      fire.flameMeshes.forEach((flame, flameIndex) => {
        ...
        if (flame.position.y > 0.72) flame.position.y = 0.05;
      ...
      if (fire.light) fire.light.intensity = (0.45 + Math.random() * 0.75) * life;
    ...
    if (this.updateAccumulator < 0.25) return;
    const tick = this.updateAccumulator;
    this.updateAccumulator = 0;
    for (const fire of this.fires) {
      if (!fire.source) continue;
      ...
      for (const candidate of this.burnables.values()) {
        if (candidate === source || candidate.burning || candidate.currentHealth <= 0) continue;
  ...
  private createTorchVisuals(): void {
  ...
  private createScorchMark(position: Vector3): void {
    if (this.scorchMarks.length >= this.maxScorches) this.scorchMarks.shift()?.dispose();
  ...
  private findBurnable(mesh: AbstractMesh): BurnableRuntime | null {
    let current: AbstractMesh | null = mesh;
    while (current) {
      const runtime = this.burnables.get(current.uniqueId);
      if (runtime) return runtime;
      current = current.parent instanceof Object && "uniqueId" in current.parent ? current.parent as AbstractMesh : null;
    }
    return null;
  }

  private disposeFire(fire: FireInstance): void {
    fire.light?.dispose();
