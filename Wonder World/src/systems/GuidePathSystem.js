...
export class GuidePathSystem {
    ...
    findFloorHeight(point) {
        ...
        const pick = this.scene.pickWithRay(new Ray(origin, new Vector3(0, -1, 0), 6.5), (mesh) => this.isGuideFloor(mesh));
        if (pick?.hit && pick.pickedPoint)
            return pick.pickedPoint.y;
        const fallback = this.scene.pickWithRay(new Ray(new Vector3(point.x, expectedY + 1.2, point.z), new Vector3(0, -1, 0), 3.5), (mesh) => mesh.checkCollisions && mesh.isEnabled() && !this.isGuideBlocker(mesh));
        if (fallback?.hit && fallback.pickedPoint)
            return fallback.pickedPoint.y;
