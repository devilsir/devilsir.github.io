...
export class AudioManager {
    ...
    applySettings(settings) {
        this.settings = settings;
        if (!this.context || !this.master || !this.effects || !this.music || !this.dialogue)
            return;
    ...
    playVoiceLikeLine(duration = 1.4) {
        const context = this.ensureContext();
        const gain = context.createGain();
        gain.gain.value = 0.025;
        const filter = context.createBiquadFilter();
    ...
    startIndustrialDepths() {
        const context = this.ensureContext();
        if (this.loops.has("industrial-depths"))
            return;
        const gain = context.createGain();
    ...
    startMachineAlarm() {
        const context = this.ensureContext();
        if (this.loops.has("machine-alarm"))
            return;
        const oscillator = context.createOscillator();
        oscillator.type = "square";
    ...
    startJesseMelody(variant, position) {
        ...
        const play = () => {
            notes.forEach((note, index) => {
                window.setTimeout(() => {
                    ...
                    if (position)
                        this.spatialTone(position, frequency, 0.28 * rate, volume, "sine");
                    else
                        this.beep(frequency, 0.28 * rate, volume, "sine", "music");
                    if (variant === "danger" && index % 2 === 0)
                        this.beep(78, 0.08, 0.025, "triangle", "effects");
    ...
    playMusicNote(index, position, reversed = false) {
        ...
        if (position)
            this.spatialTone(position, frequency, 0.42, 0.07, "sine");
        else
            this.beep(frequency, 0.42, 0.07, "sine", "music");
    }
    clubSwing(charged = false) {
        const context = this.ensureContext();
        const source = context.createBufferSource();
    ...
    clubImpact(position, material, charged = false) {
    ...
    falseVoice(variant, position, intensity = 1) {
        const context = this.ensureContext();
    ...
    mannequinJoint(position, intensity = 0.45) {
    ...
    startTemporaryBuzzer(position, duration) {
        const context = this.ensureContext();
        const oscillator = context.createOscillator();
        oscillator.type = "square";
    ...
    updateListener(position, forward, up = Vector3.Up()) {
        if (!this.context)
            return;
    ...
    createSpatialNoise(id, position, volume = 0.08) {
        if (this.spatialLoops.has(id))
            return;
        const context = this.ensureContext();
        const source = context.createBufferSource();
    ...
    spatialNoiseBurst(position, duration, volume, frequency, type) {
        const context = this.ensureContext();
        const source = context.createBufferSource();
    ...
    spatialTone(position, frequency, duration, volume, type) {
        const context = this.ensureContext();
        const oscillator = context.createOscillator();
    ...
    beep(frequency, duration, volume, type, bus) {
        const context = this.ensureContext();
        const oscillator = context.createOscillator();
        oscillator.type = type;
        oscillator.frequency.value = frequency;
    ...
    noiseBuffer(duration, amplitude) {
        const context = this.ensureContext();
