...
const waterMat=new THREE.ShaderMaterial({transparent:true,depthWrite:false,side:THREE.DoubleSide,uniforms:{uTime:{value:0},uQuality:{value:1},uSun:{value:new THREE.Vector3(.2,1,.1)},uDeep:{value:new THREE.Color(0x075675)},uShallow:{value:new THREE.Color(0x3fb6c4)}},vertexShader:`
...
fragmentShader:`
  uniform vec3 uSun,uDeep,uShallow; varying float vDepth; varying vec3 vWorld,vNormal;
  void main(){vec3 V=normalize(cameraPosition-vWorld);vec3 N=normalize(vNormal);float fres=pow(1.0-max(dot(V,N),0.0),3.0);float spec=pow(max(dot(reflect(-normalize(uSun),N),V),0.0),72.0);vec3 col=mix(uShallow,uDeep,clamp(vDepth/12.0,0.0,1.0));col=mix(col,vec3(.62,.86,.95),fres*.55)+spec*vec3(1.0,.86,.58)*1.4;gl_FragColor=vec4(col,.52+fres*.19);}`});
...
function updatePlayerCamera(dt,moving){
  const now=performance.now(),eye=new THREE.Vector3(player.pos.x,player.pos.y+(player.crouch?1.1:1.57),player.pos.z),dir=new THREE.Vector3(-Math.sin(player.yaw)*Math.cos(player.pitch),Math.sin(player.pitch),-Math.cos(player.yaw)*Math.cos(player.pitch));camera.fov=lerp(camera.fov,player.sprint?82:73,1-Math.pow(.002,dt));camera.updateProjectionMatrix();playerBody.position.set(player.pos.x,player.pos.y-(player.crouch?.24:0),player.pos.z);playerBody.rotation.y=player.yaw;playerBody.visible=state.started&&state.cameraMode!==0;firstHands.visible=state.started&&state.cameraMode===0;
...
let characterStyle={skin:2,hair:1,coat:0,accent:0,pants:0,hairStyle:0};try{Object.assign(characterStyle,JSON.parse(localStorage.qc_character||"{}"))}catch(e){}
function shadeColor(hex,light){const c=new THREE.Color(hex);c.offsetHSL(0,light>0?.015:0,light);return c}
function applyCharacterStyle(nextStyle=characterStyle){characterStyle={...characterStyle,...nextStyle};const skin=CHARACTER_OPTIONS.skin[characterStyle.skin]||CHARACTER_OPTIONS.skin[2],hair=CHARACTER_OPTIONS.hair[characterStyle.hair]||CHARACTER_OPTIONS.hair[1],coat=CHARACTER_OPTIONS.coat[characterStyle.coat]||CHARACTER_OPTIONS.coat[0],accent=CHARACTER_OPTIONS.accent[characterStyle.accent]||CHARACTER_OPTIONS.accent[0],pants=CHARACTER_OPTIONS.pants[characterStyle.pants]||CHARACTER_OPTIONS.pants[0];pSkinMat.color.set(skin);pSkinLightMat.color.copy(shadeColor(skin,.11));pSkinShadeMat.color.copy(shadeColor(skin,-.1));pHairMat.color.set(hair);pCoatMat.color.set(coat);pCoatShadeMat.color.copy(shadeColor(coat,-.13));pSleeveMat.color.set(accent);pPantsMat.color.set(pants);playerHairStyles.forEach((group,index)=>group.visible=index===characterStyle.hairStyle);syncInventoryAvatar()}
...
function updatePhysics(dt){
  player.attackCooldown=Math.max(0,player.attackCooldown-dt);player.mana=Math.min(100,player.mana+dt*(player.inWater?5:9)*(countItem("prism_pendant")?1.28:1));player.crouch=!dev.fly&&!!keys.ShiftLeft;player.sprint=!!(keys.ControlLeft||keys.ControlRight)&&!player.crouch;player.inWater=isInWater();player.inLava=isInLava();player.lavaClock=Math.max(0,player.lavaClock-dt);if(player.inLava&&player.lavaClock<=0){player.lavaClock=.62;damagePlayer(8,"lava")}const fwd=(keys.KeyW?1:0)-(keys.KeyS?1:0),side=(keys.KeyD?1:0)-(keys.KeyA?1:0),len=Math.hypot(fwd,side)||1,speed=(dev.fly?(player.sprint?11:6):player.inLava?1.55:player.inWater?2.6:player.crouch?2.1:player.sprint?7.2:4.4)*dev.speed,sy=Math.sin(player.yaw),cy=Math.cos(player.yaw);let dx=(side*cy-fwd*sy)/len*speed*dt,dz=(-side*sy-fwd*cy)/len*speed*dt;
  if(dev.fly){player.vel.set(0,0,0);player.pos.x+=dx;player.pos.z+=dz;player.pos.y+=((keys.Space?1:0)-(keys.ShiftLeft?1:0))*speed*dt;player.onGround=false;updatePlayerCamera(dt,Math.abs(dx)+Math.abs(dz)>.001);$("waterOverlay").classList.remove("on");updateVitals();return}
  player.onGround=supportedAt(player.pos);moveAxis("x",dx);moveAxis("z",dz);
  if(player.inWater){player.vel.y+=(keys.Space?8.2:2.2)*dt;player.vel.y-=player.vel.y*2.4*dt;player.vel.y=clamp(player.vel.y,-2.5,3.2)}else{player.vel.y-=18.5*dt;player.vel.y=Math.max(player.vel.y,-28)}player.onGround=false;moveAxis("y",player.vel.y*dt);
  ...
  if(!player.onGround&&player.vel.y<-.2)player.fallStart=Math.max(player.fallStart,player.pos.y-player.vel.y*.02);
...
function attackSelected(alt=false){const id=selectedItem();if(id==="wind_staff")castWind(alt);else if(id==="bow")shootArrow();else if(id==="iron_sword")meleeAttack()}
function hitEntity(e,damage,source="ataque",knock=null){if(!e||e.persistent||e.dead)return;e.hp=Math.max(0,e.hp-damage);state.combatTarget=e;state.combatTargetTime=2.8;combatParticles(new THREE.Vector3(e.x,e.y+.7,e.z),e.hostile?0xff766c:0xdce8bd,8);if(knock){e.vx=(e.vx||0)+knock.x;e.vz=(e.vz||0)+knock.z}sound("hit",.55);if(e.hp<=0)killEntity(e,source)}
const DROP_TABLE={slime:[["acid_gel",1,2]],cobrete:[["copper_core",1,1],[BLOCK.COPPER,1,2]],cristalume:[["crystal_shard",1,2],[BLOCK.CRYSTAL,1,1]],capivara:[["hide",1,2]],scorpion:[["chitin",1,2]],golem:[[BLOCK.OBSIDIAN,1,2],["gold_nugget",1,2]],wisp:[["wind_essence",1,2]],magma:[[BLOCK.BASALT,1,2],[BLOCK.SULFUR,1,1]],deer:[["hide",1,2]],owl:[["feather",1,2]]};
function killEntity(e,source){e.dead=true;combatParticles(new THREE.Vector3(e.x,e.y+.8,e.z),0xffd889,18);for(const [id,min,max] of DROP_TABLE[e.type]||[]){const lucky=countItem("lucky_charm")&&Math.random()<.28?1:0;spawnDrop(id,min+Math.floor(Math.random()*(max-min+1))+lucky,new THREE.Vector3(e.x,e.y+.5,e.z));if(id==="acid_gel")discover("antidote","Neutralização do gel ácido");if(id==="chitin")discover("chitin","Biomateriais de quitina");if(id==="wind_essence")discover("storm_focus","Energia eólica condensada")}chatMessage("COMBATE",`${e.name} derrotado com ${source}.`);removeEntity(e)}
...
function syncInventoryAvatar(){
  ...
  el.dataset.hairStyle=String(characterStyle.hairStyle||0);
}
function inventorySlotHTML(it,i,hotbar=false){return `<div class="inv-slot mc-item-slot ${hotbar&&i===state.selected?"selected":""}" draggable="${!!it}" data-index="${i}" title="${it?itemName(it.id):"Slot vazio"}">${iconHTML(it)}${hotbar?`<span class="mc-hotbar-key">${i+1}</span>`:""}</div>`}
function bindInventorySlots(){
  document.querySelectorAll("#inventory .inv-slot[data-index]").forEach(el=>{
    el.addEventListener("dragstart",e=>{dragIndex=+el.dataset.index;el.classList.add("drag-source");e.dataTransfer.effectAllowed="move"});
    ...
    el.addEventListener("drop",e=>{e.preventDefault();el.classList.remove("drag-over");const to=+el.dataset.index;if(dragIndex>=0&&to!==dragIndex)[invSlots[dragIndex],invSlots[to]]=[invSlots[to],invSlots[dragIndex]];dragIndex=-1;saveInventory();renderInventory()});
  });
}
...
function renderInventory(){
  renderHotbar();syncInventoryAvatar();
  ...
  bindInventorySlots();renderRecipes();updateRecipeBookState();
}
...
function renderCraftPreview(){
  ...
  const possible=canCraft(r);out.innerHTML=`<span class="voxel-icon" style="--icon:${itemColor(r.out[0])}"></span><span class="slot-count">${r.out[1]}</span>`;out.disabled=!possible;out.title=possible?`Criar ${r.name}`:`Faltam materiais: ${recipeNeedsLabel(r)}`;label.textContent=`${r.name} · ${r.desc}`;label.classList.toggle("unavailable",!possible);
}
function renderRecipes(){
  ...
  $("recipes").innerHTML=recipes.map((r,i)=>{const possible=canCraft(r);return `<button class="mc-recipe ${i===selectedRecipeIndex?"selected":""} ${possible?"available":"locked"}" data-recipe="${i}" type="button" title="${r.desc}"><span class="mc-recipe-icon"><span class="voxel-icon" style="--icon:${itemColor(r.out[0])}"></span><span class="slot-count">${r.out[1]}</span></span><span><strong>${r.name}</strong><small>${recipeNeedsLabel(r)}</small></span><i>${possible?"✓":""}</i></button>`}).join("");
...
function craft(i=selectedRecipeIndex){const r=recipes[i];if(!r||!canCraft(r))return toast("Faltam materiais para essa receita");for(const [id,n] of r.need)removeItem(id,n);addItem(r.out[0],r.out[1]);sound("place",.35);toast(`${r.name} criada${r.out[1]>1?"s":""}`);saveInventory();renderInventory()}
...
const CHEM=[
  ...
  {id:"chitin",name:"Compósito de quitina",eq:"quitina + cerâmica → biocompósito",need:[["chitin",2],["ceramic_plate",1]],out:[["armor_plate",1]],result:"Mantê-lo na mochila reduz parte do dano recebido."},
...
const SHOP_CATALOG={
  ...
  armor:{title:"Forja Cobalto",subtitle:"Proteções, ferramentas e peças reforçadas.",goods:[["shield",10,1],["chainmail",14,1],["armor_plate",8,1],["iron_sword",12,1],[BLOCK.BRONZE_BLOCK,9,2]]},
...
function animateEntityRig(e,dt,now,pace,dist){
  ...
  r.legs.forEach((leg,i)=>{const phase=leg.userData.phase??(i%2?Math.PI:0),s=Math.sin(gait+phase);if(e.type==="scorpion"){leg.rotation.z=leg.userData.side*(.08+s*.14);leg.rotation.y=s*.1}else{leg.rotation.x=s*(pace>0?.48:.015);leg.rotation.z=0;if(leg.userData.lower)leg.userData.lower.rotation.x=Math.max(0,-s)*.35}});
  ...
  r.wings.forEach((wing,i)=>{const side=wing.userData.side||((i%2)*2-1),flap=Math.sin(now*(e.type==="owl"?.0095:.012)+e.phase);wing.rotation.z=side*(.2+flap*.68);wing.rotation.x=flap*.08});
...
function updateCycle(dt,now){
  ...
  cloudGroup.position.x=cx+((now*.00065)%90)-45;cloudGroup.position.z=cz;cloudGroup.children.forEach(s=>s.material.opacity=(.11+.22*day)*(s.userData.base||1));waterMat.uniforms.uTime.value=now*.001;lavaMat.uniforms.uTime.value=now*.001;waterMat.uniforms.uSun.value.copy(sunPos).normalize();
