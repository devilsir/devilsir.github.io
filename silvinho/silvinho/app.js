...
const wardrobeBraName = $('#wardrobeBraName');
...
const MULTIPLAYER_SERVER_STORAGE_KEY = 'rodaRodaSilvio.multiplayerServer.v1';
...
async function connectMultiplayer(kind) {
  ...
  try {
    ...
    startLobbyPolling();
    setToast(`LOBBY ${data.lobby.code} • monte seu Silvio`, 'good');
  } catch (error) {
...
function applyRemoteSnapshot(snapshot) {
  ...
  game.players = Array.isArray(snapshot.players) ? snapshot.players.map((player, index) => ({
    ...
    score: Number(player.score) || 0,
    outfit: { hat: Number(player.outfit?.hat) || 0, glasses: Number(player.outfit?.glasses) || 0, bra: Number(player.outfit?.bra) || 0 },
...
const ACCESSORY_EDIT_STORAGE_KEY = 'rodaRodaSilvio.accessoryFits.v3';
...
function setPlayerCount(count) {
  ...
  game.players = Array.from({ length: safeCount }, (_, index) => ({
    ...
    score: 0,
    outfit: { hat: index % 6, glasses: index % 6, bra: index % 6 },
    removedAccessories: [],
  }));
  game.currentPlayerIndex = 0;
  game.round = 1;
  ...
  beginWardrobeSetup();
  updatePlayersUI();
  setToast(`${safeCount} jogador${safeCount > 1 ? 'es' : ''} • hora de montar os looks`, 'good');
}
...
function updateEditTargetUI() {
  ...
  if (accessoryEditStatus) {
    const labels = { hat: 'cabeça', glasses: 'óculos', bra: 'peitões' };
...
function setAccessoryEditTarget(category) {
  if (!['hat', 'glasses', 'bra'].includes(category)) return;
...
function setAccessoryEditing(enabled, { syncCheckbox = true } = {}) {
  ...
  if (viewerHint && game.wardrobeActive) {
    viewerHint.textContent = accessoryEditEnabled
      ...
      : `${currentWardrobePlayer()?.name?.toUpperCase() || 'JOGADOR'} • ESCOLHA CABEÇA, ÓCULOS E PEITÕES`;
  }
}
...
function renderWardrobePlayer() {
  ...
  for (const category of ['hat', 'glasses', 'bra']) {
    const list = accessoryList(category);
  ...
  if (viewerHint) viewerHint.textContent = `${player.name.toUpperCase()} • ESCOLHA CABEÇA, ÓCULOS E PEITÕES`;
}
...
wardrobeConfirmButton?.addEventListener('click', () => {
  ...
  setTimeout(() => scheduleViewerResize({ refit: true }), 80);
  startRound();
  setToast('LOOKS PRONTOS • AGORA VALE!', 'good');
});
...
const equippedAccessories = { hat: null, glasses: null, bra: null };
...
const braBodyNodes = Array.isArray(config.accessoryOcclusion?.braBodyNodes)
  ? config.accessoryOcclusion.braBodyNodes
...
function setHatHairOcclusion() {
  // A cabeça/cabelo do Silvio deve permanecer exatamente como no GLB original.
...
function prepareBodyStencilForBarriers() {
  ...
  // Bit 2 = sutiãs/tops. Só torso + seios testam esse bit, então braços e
...
function prepareAccessoryAntiClipping(instance, category) {
  ...
  for (const mesh of meshes) {
    ...
    const visibleMaterials = materials.map((material) => {
      ...
      clone.polygonOffsetFactor = category === 'bra' ? -3.5 : -2.0;
      clone.polygonOffsetUnits = category === 'bra' ? -5.0 : -3.0;
      clone.needsUpdate = true;
      return clone;
    });
...
function accessoryCategoryLabel(category) {
  ...
  if (category === 'bra') return 'o sutiã';
  return 'um acessório';
}
...
function applyPlayerOutfit(player) {
  if (!modelRoot || !player || game.finale) return;
...
function startFinale({ remote = false } = {}) {
  ...
  finalAudio = new Audio(config.finalAudio || './assets/audio/parabens-final.wav');
...
const modelUrl = new URL(config.modelPath || './assets/model/silvio.glb', window.location.href).href;
loader.load(
  modelUrl,
  (gltf) => {
    ...
    const previewPlayer = game.wardrobeActive
      ? game.players[game.wardrobePlayerIndex]
      : currentPlayer();
    ...
    viewerHint.textContent = game.wardrobeActive
      ...
      : 'GIRE O CORPO • OS PEITÕES RESPONDEM À VELOCIDADE DO MOVIMENTO';
  },
  (progress) => {
    if (!loadingText || !progress.total) return;
    loadingText.textContent = `Carregando Silvio… ${Math.round(progress.loaded / progress.total * 100)}%`;
  },
  (error) => {
    console.error('Falha ao carregar o modelo:', modelUrl, error);
    if (loadingText) loadingText.textContent = 'Servidor desconectado. Feche e abra novamente pelo start.bat.';
  },
);
