...
function processPetAttention(dragging) {
  ...
  living.record('pet', { kind: dragging ? 'back' : 'calm' });
...
async function toggleSleep(forceValue) {
  ...
  try {
    ...
    living.record(sleeping ? 'sleep' : 'wake', {}, { timeline: sleeping });
...
function registerServiceWorker() {
  if ('serviceWorker' in navigator && location.protocol.startsWith('http')) {
    navigator.serviceWorker.register('sw.js?v=18-premium-life-drawer')
