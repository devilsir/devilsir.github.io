      ...
      function approxRadius(pmNeutral, knownCharge, knownIonicPm, targetCharge) {
        if (targetCharge === 0) return pmNeutral;
        if (knownCharge && targetCharge === knownCharge && knownIonicPm) return knownIonicPm;
      ...
      function AtomicIonicRadiusApp() {
        ...
        function playTone(type = 'click', opts = {}) {
          ...
          o.connect(g);
          g.connect(f);
          if (p) { f.connect(p); p.connect(ctx.destination); } else { f.connect(ctx.destination); }
          ...
          if (p) p.pan.setValueAtTime(t.pan, now);
        ...
        function onToggleView(newView) {
          setView(newView);
          if (newView === 'neutral') playTone('neutral');
          else if (newView === 'ion') playTone(currentCharge > 0 ? 'cation' : currentCharge < 0 ? 'anion' : 'neutral');
          else playTone('toggle');
        }
        ...
        function nudgeElectrons(delta) {
          ...
          setElectronCount(e => {
            ...
            if (next !== e) playTone(delta < 0 ? 'cation' : 'anion');
            return next;
          });
        }
        ...
        function currentRadiusPm() {
          return approxRadius(
            ...
            ((selected.ionOptions && selected.ionOptions[ionIndex]) || selected.ion).charge,
            ((selected.ionOptions && selected.ionOptions[ionIndex]) || selected.ion).ionicRadius,
            currentCharge
          );
        }
        ...
        function AtomView({ mode }) {
          ...
          return (
            <div className="flex flex-col items-center gap-2">
              <div className="text-sm text-white/80">{lbl} <span className="text-white/50">({Math.round(pmVar)} pm)</span></div>
              <div style={{ width: svgSize, height: svgSize }} className="flex items-center justify-center">
                <svg width={svgSize} height={svgSize} viewBox={`${-svgSize/2} ${-svgSize/2} ${svgSize} ${svgSize}`}
                     style={{ transition: `all ${animMs}ms cubic-bezier(.2,.8,.2,1)` }}>
                  {/* Anéis e elétrons animados */}
                  {[...Array(nShells)].map((_,i) => {
                    ...
                    return (
                      <g key={`shell-${idx}`} className="shellGroup">
                        <circle cx={0} cy={0} r={rr} fill="none" stroke="rgba(255,255,255,0.6)" strokeDasharray="4 6" strokeWidth={1}
                                className="ring" style={{ animationDuration: `${3 + idx*0.5}s` }} />
                        {eOnShell > 0 && (
                          <g className="orbit" style={{ animation: `spin ${speed}s linear infinite`, animationDirection: dir, transformOrigin: 'center', transformBox: 'fill-box' }}>
                            {Array.from({ length: eOnShell }).map((__,k) => {
                              const ang = (k / eOnShell) * Math.PI * 2;
                              const x = rr * Math.cos(ang);
                              const y = rr * Math.sin(ang);
                              const er = Math.max(2, rPx * 0.025);
                              return <circle key={`e-${idx}-${k}-${eCount}`} cx={x} cy={y} r={er} className="electron" />;
                            })}
                          </g>
                        )}
                      </g>
                  ...
                  {/* Núcleo */}
                  <g className="nucleus" style={{ animation: 'pulse 4.5s ease-in-out infinite', transformOrigin: 'center', transformBox: 'fill-box' }}>
                    <circle cx={0} cy={0} r={Math.max(6, rPx * 0.10)} fill="#ffffff" opacity={0.9} />
                  </g>

                  {/* Corpo do átomo/íon com pulsação leve */}
                  <g className="atomBody" style={{ animation: 'pulse 6s ease-in-out infinite', transformOrigin: 'center', transformBox: 'fill-box' }}>
                    <circle cx={0} cy={0} r={rPx}
                            fill={color}
                            opacity={0.8}
                            stroke={border}
                            strokeWidth={4}
        ...
        return (
          <div className="min-h-screen w-full bg-gradient-to-br from-slate-900 via-slate-950 to-black text-white p-4 md:p-8">
            {/* Header */}
            <header className="flex flex-col md:flex-row md:items-end gap-4 md:gap-8 mb-6">
              <div className="flex-1">
                <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Laboratório de Raios: <span className="text-sky-300">Atômico</span> & <span className="text-emerald-300">Iônico</span></h1>
                <p className="text-white/70 text-sm md:text-base mt-1">Visualize como o raio muda quando o átomo vira íon. Controle o número de elétrons e ouça feedback sonoro. ✨</p>
              </div>
              <div className="flex items-center gap-3">
                <button onClick={() => setSoundOn(s => !s)}
                        className={`px-3 py-2 rounded-xl text-sm font-medium border ${soundOn ? 'bg-white/10 border-white/30' : 'bg-black/30 border-white/10'} hover:bg-white/15 transition`}>{soundOn ? 'Som: ligado' : 'Som: desligado'}</button>
                <button onClick={() => onToggleView(view === 'compare' ? 'neutral' : view === 'neutral' ? 'ion' : 'compare')}
                        className="px-3 py-2 rounded-xl text-sm font-medium border bg-white/10 border-white/30 hover:bg-white/15 transition">Modo: {view === 'compare' ? 'Comparar' : view === 'neutral' ? 'Neutro' : 'Íon'}</button>
              </div>
            </header>
            ...
            <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2">
                <div className="rounded-2xl p-5 border border-white/10 bg-white/5 backdrop-blur-sm shadow-xl">
                  {/* Seletor de elemento + ajustes */}
                  <div className="flex flex-col md:flex-row md:items-end gap-4 mb-5">
                    <div className="flex-1">
                      <label className="block text-xs uppercase tracking-wider text-white/60 mb-1">Elemento</label>
                      <div className="flex flex-wrap gap-2">
                        {ELEMENTS.map(el => (
                          <button key={el.symbol}
                                  onClick={() => onSelectSymbol(el.symbol)}
                                  className={`px-3 py-2 rounded-xl text-sm border transition ${selected.symbol === el.symbol ? 'bg-sky-500/20 border-sky-300/40' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                            {el.symbol}
                          </button>
                        ))}
                      </div>
                      <div className="mt-2 text-white/70 text-sm">Selecionado: <span className="font-semibold">{selected.name} ({selected.symbol})</span> — Período {selected.period}, Z = {selected.Z}</div>
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs uppercase tracking-wider text-white/60 mb-1">Escala</label>
                        <div className="flex items-center gap-2">
                          <input id="absScale" type="checkbox" className="accent-sky-400" checked={absoluteScale} onChange={e => { setAbsoluteScale(e.target.checked); playTone('toggle'); }} />
                          <label htmlFor="absScale" className="text-sm text-white/80">Absoluta (comparar elementos)</label>
                        </div>
                      </div>
                      <div>
                        <label className="block text-xs uppercase tracking-wider text-white/60 mb-1">Velocidade</label>
                        <input type="range" min={300} max={1600} step={50} value={animMs} onChange={e => setAnimMs(Number(e.target.value))} className="w-full" />
                        <div className="text-xs text-white/60">{animMs} ms</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mb-6 p-3 rounded-xl bg-white/5 border border-white/10">
                    <div className="flex items-end justify-between gap-4 flex-wrap">
                      <div className="min-w-[220px]">
                        <div className="text-xs uppercase tracking-wider text-white/60 mb-1">Elétrons (e⁻)</div>
                        <div className="flex items-center gap-2">
                          <button className="px-3 py-2 rounded-lg bg-white/10 border border-white/15 hover:bg-white/15" onClick={() => nudgeElectrons(-1)}>-</button>
                          <input type="number" className="w-24 text-black rounded-lg px-2 py-1" value={electronCount}
                                 min={0} max={selected.Z + 5}
                                 onChange={e => setElectronCount(clamp(Number(e.target.value||0), 0, selected.Z + 5))} />
                          <button className="px-3 py-2 rounded-lg bg-white/10 border border-white/15 hover:bg-white/15" onClick={() => nudgeElectrons(+1)}>+</button>
                        </div>
                        <input type="range" className="w-full mt-2" min={0} max={selected.Z + 5} step={1}
                               value={electronCount} onChange={e => setElectronCount(Number(e.target.value))} />
                      </div>

                      <div className="flex-1 grid grid-cols-2 gap-4 min-w-[240px]">
                        <div className="p-2 rounded-lg bg-white/5 border border-white/10 text-sm">
                          <div className="text-white/60">Carga (Z − e⁻)</div>
                          <div className="text-white text-lg font-semibold">{currentCharge === 0 ? '0 (neutro)' : (currentCharge > 0 ? `+${currentCharge}` : `${currentCharge}`)}</div>
                          <div className="text-white/60 text-xs">Rótulo: <span className="text-white/80 font-semibold">{formatIonLabel(selected.symbol, currentCharge)}</span></div>
                          {selected.ionOptions && selected.ionOptions.length > 1 && (
                            <div className="mt-2 flex items-center gap-2">
                              <span className="text-white/60 text-xs">Íon de referência:</span>
                              <select className="text-black rounded px-2 py-1" value={ionIndex} onChange={e => { setIonIndex(Number(e.target.value)); playTone('toggle'); }}>
                                {selected.ionOptions.map((io, idx) => (
                                  <option key={io.label} value={idx}>{io.label} ({io.ionicRadius} pm)</option>
                                ))}
                              </select>
                            </div>
                          )}
                        ...
                        <div className="p-2 rounded-lg bg-white/5 border border-white/10 text-sm">
                          <div className="text-white/60">Raio atual</div>
                          <div className="text-white text-lg font-semibold">{Math.round(currentRadiusPm())} pm</div>
                        </div>
                      </div>

                      <div className="flex items-center gap-2">
                        <button className="px-3 py-2 rounded-lg bg-white/10 border border-white/15 hover:bg-white/15" onClick={() => { setElectronCount(selected.Z); playTone('neutral'); }}>Neutro</button>
                        <button className="px-3 py-2 rounded-lg bg-white/10 border border-white/15 hover:bg-white/15" onClick={() => { const ion = (selected.ionOptions && selected.ionOptions[ionIndex]) || selected.ion; setElectronCount(selected.Z - ion.charge); playTone('success'); }}>Íon comum</button>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Visualização */}
                  <div className="mt-2">
                    {view === 'compare' ? <ComparePanel /> : <AtomView mode={view} />}
                  </div>

                  {/* Legendas */}
                  <div className="mt-6 grid grid-cols-1 md:grid-cols-1 gap-4 text-sm text-white/80">
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <div className="font-semibold mb-1">Tendências periódicas</div>
                      <ul className="list-disc pl-5 space-y-1 text-white/70">
                        <li>Descendo um grupo: raio <span className="text-emerald-300">aumenta</span>.</li>
                        <li>Da esquerda para a direita: raio <span className="text-rose-300">diminui</span>.</li>
                        <li>Cátion (perde elétrons): raio <span className="text-rose-300">menor</span>.</li>
                        <li>Ânion (ganha elétrons): raio <span className="text-emerald-300">maior</span>.</li>
                      </ul>
                    </div>
                  </div>
                </div>
              ...
              {/* Aside: exemplos, áudio e testes */}
              <aside className="space-y-6">
                <div className="rounded-2xl p-5 border border-white/10 bg-white/5 backdrop-blur-sm shadow-xl">
                  <div className="text-sm uppercase tracking-wider text-white/60 mb-3">Exemplos rápidos</div>
                  <div className="grid grid-cols-2 gap-3">
                    {QUICK.map(sym => (
                      <button key={sym}
                              onClick={() => { const el = ELEMENTS.find(e => e.symbol===sym); const firstIon = (el?.ionOptions && el.ionOptions[0]) || el?.ion; onSelectSymbol(sym); setView('compare'); setElectronCount((el?.Z || 0) - (firstIon?.charge || 0)); }}
                              className="text-left p-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 transition">
                        <div className="text-base font-semibold">{sym}</div>
                        <div className="text-xs text-white/60">{(() => { const el = ELEMENTS.find(e => e.symbol===sym); const firstIon = (el?.ionOptions && el.ionOptions[0]) || el?.ion; return `${sym} → ${firstIon?.label || ''}`; })()}</div>
                      </button>
                    ))}
                  </div>
                  <div className="mt-4 text-xs text-white/60">Clique para carregar, ajustar para o íon comum e comparar ao neutro.</div>
                </div>
              </aside>
            </section>
            ...
            <footer className="mt-8 text-center text-xs text-white/50">
              Feito com ♥ para aulas de Química. Valores aproximados para fins educacionais.
            </footer>
      ...
      root.render(<AtomicIonicRadiusApp />);
